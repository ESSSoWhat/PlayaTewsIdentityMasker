...
class TorchDeviceInfo:
    ...
    def __eq__(self, other):
        if (
            ...
            and isinstance(self, TorchDeviceInfo)
            and isinstance(other, TorchDeviceInfo)
        ):
    ...
    def __str__(self):
        if self.is_cpu():
            ...
            return (
                f"[{self._index}] {self._name} [{(self._total_memory / 1024**3) :.3}Gb]"
            )
...
def get_available_devices_info(
    include_cpu=True, cpu_only=False
) -> List[TorchDeviceInfo]:
    """
