...
def rct(
    ...
    target_l = target[..., 0]
    target_l = ne.evaluate(
        "(target_l - target_l_mean) * source_l_std / target_l_std + source_l_mean"
    )

    target_a = target[..., 1]
    target_a = ne.evaluate(
        "(target_a - target_a_mean) * source_a_std / target_a_std + source_a_mean"
    )

    target_b = target[..., 2]
    target_b = ne.evaluate(
        "(target_b - target_b_mean) * source_b_std / target_b_std + source_b_mean"
    )
