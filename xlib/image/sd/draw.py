...
def bezier(wh, A, B, C):
    ...
    hl_t = np.clip(
        np.stack((hl_m + hl_m, -hl_n - hl_m, hl_n - hl_m), -1) * hl_z[..., None] - kx,
        0.0,
        1.0,
    )
...
def random_circle_faded(wh, rnd_state=None):
    ...
    return circle_faded(
        wh, [rnd_state.randint(h), rnd_state.randint(w)], [fade_start, fade_end]
    )
...
def random_bezier_split_faded(wh):
    ...
    A = center + radius * np.float32(
        [math.sin(degA * deg_2_rad), math.cos(degA * deg_2_rad)]
    )
    B = center + np.random.randint(radius) * np.float32(
        [math.sin(degB * deg_2_rad), math.cos(degB * deg_2_rad)]
    )
    C = center + radius * np.float32(
        [math.sin(degC * deg_2_rad), math.cos(degC * deg_2_rad)]
    )
