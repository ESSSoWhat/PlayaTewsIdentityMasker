...
class ImageProcessor:
    ...
    def gamma(
        self, red: float, green: float, blue: float, mask=None
    ) -> "ImageProcessor":
    ...
    def gaussian_sharpen(
        self, sigma: float, power: float, mask=None
    ) -> "ImageProcessor":
        """
        sigma  float
        ...
        img = cv2.addWeighted(
            img, 1.0 + power, cv2.GaussianBlur(img, (0, 0), sigma), -power, 0
        )
        img = np.clip(img, 0, 1, out=img)
        img = img.reshape((H, W, N, C)).transpose((2, 0, 1, 3))
    ...
    def gaussian_blur(
        self, sigma: float, opacity: float = 1.0, mask=None
    ) -> "ImageProcessor":
        """
        sigma  float
    ...
    def median_blur(
        self, size: int, opacity: float = 1.0, mask=None
    ) -> "ImageProcessor":
        """
    ...
    def erode_blur(
        self, erode: int, blur: int, fade_to_border: bool = False
    ) -> "ImageProcessor":
        """
    ...
    def levels(self, in_bwg_out_bw, mask=None) -> "ImageProcessor":
        ...
        if N != VN or C != VC or VD != 5:
            raise ValueError(
                "wrong in_bwg_out_bw size. Must have 5 floats at last dim."
            )
    ...
    def hsv(
        self, h_diff: float, s_diff: float, v_diff: float, mask=None
    ) -> "ImageProcessor":
        """
    ...
    def jpeg_recompress(self, quality: int, mask=None) -> "ImageProcessor":
        ...
        for x in img:
            ret, result = cv2.imencode(
                ".jpg", x, [int(cv2.IMWRITE_JPEG_QUALITY), quality]
            )
            if not ret:
    ...
    def rct(
        ...
        (
            ...
            img_for_stat[..., 0].mean((1, 2), keepdims=True),
            img_for_stat[..., 0].std((1, 2), keepdims=True),
            img_for_stat[..., 1].mean((1, 2), keepdims=True),
            img_for_stat[..., 1].std((1, 2), keepdims=True),
            img_for_stat[..., 2].mean((1, 2), keepdims=True),
            img_for_stat[..., 2].std((1, 2), keepdims=True),
        )
        ...
        (
            ...
            like_for_stat[..., 0].mean((1, 2), keepdims=True),
            like_for_stat[..., 0].std((1, 2), keepdims=True),
            like_for_stat[..., 1].mean((1, 2), keepdims=True),
            like_for_stat[..., 1].std((1, 2), keepdims=True),
            like_for_stat[..., 2].mean((1, 2), keepdims=True),
            like_for_stat[..., 2].std((1, 2), keepdims=True),
        )

        # not as in the paper: scale by the standard deviations using reciprocal of paper proposed factor
        source_l = img[..., 0]
        source_l = ne.evaluate(
            "(source_l - source_l_mean) * like_l_std / source_l_std + like_l_mean"
        )

        source_a = img[..., 1]
        source_a = ne.evaluate(
            "(source_a - source_a_mean) * like_a_std / source_a_std + like_a_mean"
        )

        source_b = img[..., 2]
        source_b = ne.evaluate(
            "(source_b - source_b_mean) * like_b_std / source_b_std + like_b_mean"
        )
    ...
    def to_grayscale(self) -> "ImageProcessor":
        ...
        if C != 1:
            ...
            if C == 2:
                ...
                img = np.dot(img, np.array([0.1140, 0.5870, 0.2989], np.float32))[
                    ..., None
                ]
    ...
    def resize(
        self, size: Tuple, interpolation: "ImageProcessor.Interpolation" = None
    ) -> "ImageProcessor":
        """
        resize to (W,H)
        """
    ...
    def warp_affine(
        ...
        img = cv2.warpAffine(
            img, mat, (out_width, out_height), flags=_cv_inter[interpolation]
        )
...
_cv_inter = {
    ImageProcessor.Interpolation.NEAREST: cv2.INTER_NEAREST,
