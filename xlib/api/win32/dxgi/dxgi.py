...
class IDXGIObject(IUnknown):
    ...
    def SetPrivateDataInterface(
        self, guid: REFGUID, interface: IUnknown
    ) -> HRESULT: ...

    def GetPrivateData(
        self, guid: REFGUID, in_out_data_size: POINTER(c_uint), out_data: c_void_p
    ) -> HRESULT: ...
...
class IDXGIAdapter(IDXGIObject):
    def EnumOutputs(
        self, output: c_uint, output_interface: POINTER(c_void_p)
    ) -> HRESULT: ...  ## IDXGIOutput**

    def GetDesc(self, out_desc: POINTER(DXGI_ADAPTER_DESC)) -> HRESULT: ...

    def CheckInterfaceSupport(
        self, guid: REFGUID, out_umdversion: POINTER(c_longlong)
    ) -> HRESULT: ...  # InterfaceName, pUMDVersion
...
class IDXGIFactory1(IDXGIFactory):
    def EnumAdapters1(
        self, adapter_id: c_uint, out_interface: POINTER(IDXGIAdapter1)
    ) -> HRESULT: ...
...
@dll_import("dxgi")
def CreateDXGIFactory1(
    refiid: REFIID, p_IDXGIFactory1: POINTER(IDXGIFactory1)
) -> HRESULT: ...


@dll_import("dxgi")
def CreateDXGIFactory2(
    flags: c_uint, refiid: REFIID, p_IDXGIFactory2: POINTER(IDXGIFactory2)
) -> HRESULT: ...
