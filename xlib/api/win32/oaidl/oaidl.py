...
from ..wintypes import DWORD, GUID, HRESULT, LPCOLESTR, VARIANT, IUnknown, interface


@interface
class IErrorLog(IUnknown):
    def AddError(
        self, pszPropName: LPCOLESTR, pExcepInfo: c_void_p
    ) -> HRESULT: ...  # EXCEPINFO
...
class IPropertyBag(IUnknown):
    def Read(
        self, pszPropName: LPCOLESTR, pVar: POINTER(VARIANT), pErrorLog: IErrorLog
    ) -> HRESULT: ...
