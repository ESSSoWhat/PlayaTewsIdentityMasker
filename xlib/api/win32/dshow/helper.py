...
from .. import oaidl, objidl, ole32, strmif, uuids, wintypes
...
def get_video_input_devices_names() -> List[str]:
    ...
    if (
        ole32.CoCreateInstance(
            uuids.CLSID_SystemDeviceEnum,
            None,
            ole32.CLSCTX.CLSCTX_INPROC_SERVER,
            strmif.ICreateDevEnum.IID,
            sys_dev_enum,
        )
        == wintypes.ERROR.SUCCESS
    ):
        pEnumCat = objidl.IEnumMoniker()

        if (
            sys_dev_enum.CreateClassEnumerator(
                uuids.CLSID_VideoInputDeviceCategory, pEnumCat, 0
            )
            == wintypes.ERROR.SUCCESS
        ):
            ...
            while pEnumCat.Next(1, moniker, None) == wintypes.ERROR.SUCCESS:
                ...
                if (
                    moniker.BindToStorage(None, None, oaidl.IPropertyBag.IID, prop_bag)
                    == wintypes.ERROR.SUCCESS
                ):
                    ...
                    if hr != wintypes.ERROR.SUCCESS:
                        hr = prop_bag.Read(
                            wintypes.LPCOLESTR("FriendlyName"), var, None
                        )
                    if hr == wintypes.ERROR.SUCCESS:
