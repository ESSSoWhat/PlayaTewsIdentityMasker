...
class QXCollapsibleSection(QXFrame):
    ...
    def _on_registered(self):
        ...
        self._is_opened = self.get_widget_data(
            (QXCollapsibleSection, "opened"), default_value=self._is_opened
        )
        if self._is_opened:
            self.open()
        else:
