...
class Device:
    ...
    def __init__(self, device_info: DeviceInfo, **kwargs):
        if kwargs.get("_check", None) is None:
            raise Exception(
                "You should not to create Device from constructor. Use get_device()"
            )
    ...
    def _get_ctx(self) -> CL.cl_context:
        ...
        if self._ctx is None:
            clr = CL.CLRESULT()
            ctx = CL.clCreateContext(
                None, 1, (CL.cl_device_id * 1)(self._device_id), None, None, clr
            )
    ...
    def _get_ctx_q(self) -> CL.cl_command_queue:
        ...
        if self._ctx_q is None:
            clr = CL.CLRESULT()
            ctx_q = CL.clCreateCommandQueue(
                self._get_ctx(), self._device_id, CL.cl_command_queue_properties(0), clr
            )
            if clr != CL.CLERROR.SUCCESS:
    ...
    def _compile_kernel(self, key, kernel_text) -> CL.cl_kernel:
        ...
        if compiled_krn is None:
            ...
            prog = CL.clCreateProgramWithSource(
                self._get_ctx(), 1, CL.c_char_p(kernel_text.encode()), None, clr
            )
            if clr != CL.CLERROR.SUCCESS:
                raise Exception(
                    f"clCreateProgramWithSource error {clr}, with kernel_text:\n{kernel_text}"
                )

            clr = CL.clBuildProgram(
                prog,
                1,
                (CL.cl_device_id * 1)(self._device_id),
                CL.c_char_p("-cl-std=CL1.2 -cl-single-precision-constant".encode()),
                None,
                None,
            )
            if clr != CL.CLERROR.SUCCESS:
                build_log_size = CL.c_size_t()
                clr = CL.clGetProgramBuildInfo(
                    prog,
                    self._device_id,
                    CL.CL_PROGRAM_BUILD_LOG,
                    0,
                    None,
                    CL.byref(build_log_size),
                )
                if clr != CL.CLERROR.SUCCESS:
            ...
            if num_kernels.value != 1:
                raise Exception(
                    f"Kernel must contain only one __kernel:\n\n{kernel_text}"
                )
    ...
    def _cl_mem_alloc(self, size) -> CL.cl_mem:
        ...
        if clr == CL.CLERROR.SUCCESS:
            ...
            ev = CL.cl_event()
            clr = CL.clEnqueueFillBuffer(
                self._get_ctx_q(), mem, (CL.c_char * 1)(), 1, 0, 1, 0, None, ev
            )
            if clr == CL.CLERROR.SUCCESS:
    ...
    def _cl_mem_free(self, mem: CL.cl_mem):
        size = CL.c_size_t()
        clr = CL.clGetMemObjectInfo(
            mem, CL.CL_MEM_SIZE, CL.sizeof(size), CL.byref(size), None
        )
        if clr != CL.CLERROR.SUCCESS:
            ...
            while True:
                ...
                if mem is None:
                    ...
                    if not self._release_random_pooled_buffers():
                        raise Exception(
                            f"Unable to allocate {size // 1024**2}Mb on {self.get_description()}"
                        )
                    continue
                break
    ...
    def cleanup(self):
        ...
        if self._total_memory_allocated != 0:
            raise Exception(
                "Unable to cleanup CLDevice, while not all Buffers are deallocated."
            )
        ...
        *args,
        ...
        for i, arg in enumerate(args):
            if isinstance(arg, Buffer):
                ...
                if cl_type is None:
                    raise ValueError(
                        f"Cannot convert type {arg.__class__} to OpenCL type."
                    )
                arg = cl_type(arg)
            krn_args.append(arg)
        ...
        clr = CL.clEnqueueNDRangeKernel(
            self._get_ctx_q(),
            ckernel,
            work_dim,
            global_shape_offsets_ar,
            global_shape_ar,
            local_shape_ar,
            0,
            None,
...
def _get_opencl_device_ids() -> List[CL.cl_device_id]:
    ...
    if _opencl_device_ids is None:
        ...
        while True:
            ...
            if (
                CL.clGetPlatformIDs(0, None, num_platforms) != CL.CLERROR.SUCCESS
                or num_platforms.value == 0
            ):
                break
            ...
            for i_platform in range(num_platforms.value):
                ...
                if (
                    CL.clGetDeviceIDs(platform, device_types, 0, None, num_devices)
                    != CL.CLERROR.SUCCESS
                    or num_devices.value == 0
                ):
                    continue
                ...
                if (
                    CL.clGetDeviceIDs(
                        platform, device_types, num_devices.value, device_ids, None
                    )
                    != CL.CLERROR.SUCCESS
                ):
                    continue
                ...
                for i in range(num_devices.value):
                    ...
                    if device_id is not None:
                        ...
                        if (
                            ...
                            device_version = CL.create_string_buffer(
                                device_version_size.value
                            )
                            if (
                                CL.clGetDeviceInfo(
                                    device_id,
...
def get_available_devices_info() -> List[DeviceInfo]:
    ...
    for device_index, device_id in enumerate(_get_opencl_device_ids()):
        ...
        if (
            CL.clGetDeviceInfo(device_id, CL.CL_DEVICE_NAME, 0, None, name_size)
            == CL.CLERROR.SUCCESS
        ):
            name_value = CL.create_string_buffer(name_size.value)
            if (
                CL.clGetDeviceInfo(
                    device_id, CL.CL_DEVICE_NAME, name_size.value, name_value, None
                )
                == CL.CLERROR.SUCCESS
            ):
        ...
        if (
            CL.clGetDeviceInfo(
                ...
                CL.sizeof(global_mem_size),
                CL.byref(global_mem_size),
                None,
            )
            == CL.CLERROR.SUCCESS
        ):
        ...
        CL.clGetDeviceInfo(
            ...
            CL.sizeof(max_compute_units),
            CL.byref(max_compute_units),
            None,
        )
