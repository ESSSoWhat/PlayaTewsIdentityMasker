...
class Buffer:
    ...
    def has_cl_mem(self) -> bool:
        return self._cl_mem is not None
        ...
        if isinstance(value, Buffer):
            if self != value:
                if self._size != value._size:
                    raise Exception(
                        f"Unable to copy from Buffer with {value._size} size to buffer with {self._size} size."
                    )

                if self._device == value._device:
                    CL.clEnqueueCopyBuffer(
                        self._device._get_ctx_q(),
                        value.get_cl_mem(),
                        self.get_cl_mem(),
                        0,
                        0,
            ...
            if value.nbytes != self._size:
                raise ValueError(
                    f"Value size {value.nbytes} does not match Buffer size {self._size}."
                )
            ...
            clr = CL.clEnqueueWriteBuffer(
                self._device._get_ctx_q(),
                self.get_cl_mem(),
                False,
                0,
                value.nbytes,
                value.ctypes.data_as(CL.c_void_p),
                0,
                None,
                ev,
            )
    ...
    def np(self, shape: Iterable, dtype: np.dtype, out=None):
        ...
        if out.nbytes != self._size:
            raise ValueError(
                f"Unable to represent Buffer with size {self._size} as shape {shape} with dtype {dtype}"
            )

        clr = CL.clEnqueueReadBuffer(
            self._device._get_ctx_q(),
            self.get_cl_mem(),
            True,
            0,
