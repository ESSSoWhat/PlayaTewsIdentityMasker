...
def clGetPlatformIDs(
    num_entries: cl_uint,
    platforms: POINTER(cl_platform_id),
    num_platforms: POINTER(cl_uint),
) -> CLRESULT: ...


@dll_import("OpenCL")
def clGetPlatformInfo(
...
def clGetDeviceIDs(
    ...
    num_entries: cl_uint,
    devices: POINTER(cl_device_id),
    num_devices: POINTER(cl_uint),
) -> CLRESULT: ...
...
def clEnqueueReadBuffer(
    ...
    event_wait_list: POINTER(cl_event),
    event: POINTER(cl_event),
) -> CLRESULT: ...
...
@dll_import("OpenCL")
def clEnqueueFillBuffer(
    command_queue: cl_command_queue,
    buffer: cl_mem,
    pattern: c_void_p,
    pattern_size: c_size_t,
    offset: c_size_t,
    size: c_size_t,
    num_events_in_wait_list: cl_uint,
    event_wait_list: POINTER(cl_event),
...
@dll_import("OpenCL")
def clCreateKernelsInProgram(
    program: cl_program,
    num_kernels: cl_uint,
    kernels: POINTER(cl_kernel),
    num_kernels_ret: POINTER(cl_uint),
) -> CLRESULT: ...
...
@dll_import("OpenCL")
def clSetKernelArg(
    kernel: cl_kernel, arg_index: cl_uint, arg_size: c_size_t, arg_value: c_void_p
) -> CLRESULT: ...
...
def clEnqueueNDRangeKernel(
    ...
    kernel: cl_kernel,
    work_dim: cl_uint,
    global_work_offset: POINTER(c_size_t),
