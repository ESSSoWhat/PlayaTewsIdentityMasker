...
def _all_close(x, y, atol=1, btol=1):
    return np.allclose(
        np.ndarray.flatten(x[None, ...]), np.ndarray.flatten(y[None, ...]), atol, btol
    )
...
def rct_test():
    for _ in range(10):
        for dtype in [np.float16, np.float32]:
            ...
            mask_t = Tensor(
                shape=mask_shape, dtype=dtype, initializer=InitRandomUniform()
            )

            result_t = op.rct(
                target_t, source_t, target_mask_t=mask_t, source_mask_t=mask_t
            )
            ...
            for in_mode in ["RGB", "BGR", "XYZ", "LAB"]:
                for out_mode in ["RGB", "BGR", "XYZ", "LAB"]:
                    for dtype in [np.float16, np.float32]:
                        ...
                        print(
                            f"cvt_color {shape} {str(np.dtype(dtype).name)} {in_mode}->{out_mode} ... ",
                            end="",
                            flush=True,
                        )
                        ...
                        out_t = op.cvt_color(
                            inp_t, in_mode=in_mode, out_mode=out_mode, ch_axis=ch_axis
                        )
                        inp_t2 = op.cvt_color(
                            out_t, in_mode=out_mode, out_mode=in_mode, ch_axis=ch_axis
                        )

                        is_check = in_mode in ["RGB", "BGR", "XYZ"] and out_mode in [
                            "XYZ",
                            "LAB",
                        ]

                        if is_check and not _all_close(
                            inp_t.np(), inp_t2.np(), atol=0.1, btol=0.1
                        ):
...
def cast_test():
    for in_dtype in HType.get_np_scalar_types():
        for out_dtype in HType.get_np_scalar_types():
            ...
            print(
                f"cast: {shape} in_dtype:{str(np.dtype(in_dtype).name)} out_dtype:{str(np.dtype(out_dtype).name)}  ... ",
                end="",
                flush=True,
            )
...
def binary_morph_test():
    for shape_len in range(2, 4):
        for dtype in HType.get_np_scalar_types():
            ...
            print(
                f"binary_morph: {shape} erode_dilate:{erode_dilate} blur:{blur} {np.dtype(dtype).name} ... ",
                end="",
                flush=True,
            )

            op.binary_morph(
                input_t, erode_dilate=erode_dilate, blur=blur, fade_to_border=True
            )
...
def binary_erode_circle_test():
    for shape_len in range(2, 4):
        for dtype in HType.get_np_scalar_types():
            ...
            print(
                f"binary_erode_circle: {shape} radius:{radius} iters:{iterations} {np.dtype(dtype).name} ... ",
                end="",
                flush=True,
            )
...
def binary_dilate_circle_test():
    for shape_len in range(2, 4):
        for dtype in HType.get_np_scalar_types():
            ...
            print(
                f"binary_dilate_circle: {shape} radius:{radius} iters:{iterations} {np.dtype(dtype).name} ... ",
                end="",
                flush=True,
            )
...
def gaussian_blur_test():
    for shape_len in range(2, 5):
        for dtype in [np.float16, np.float32]:
            ...
            print(
                f"gaussian_blur: {shape} sigma:{sigma} {np.dtype(dtype).name} ... ",
                end="",
                flush=True,
            )
            ...
            for mode in ["constant"]:
                for dtype in HType.get_np_scalar_types():
                    while True:
                        ...
                        paddings = tuple(
                            (np.random.randint(8), np.random.randint(8))
                            for i in range(len(shape))
                        )

                        print(
                            f"pad: {shape} {paddings} {mode} {np.dtype(dtype).name} ... ",
                            end="",
                            flush=True,
                        )
                        ...
                        print(
                            f"{pad_n.shape} == {pad_t.shape} ... ", end="", flush=True
                        )
        ...
        for shape_len in range(5, 1, -1):
            for dtype in HType.get_np_scalar_types():
                while True:
                    ...
                    print(
                        f"slice_set: {shape} {np.dtype(dtype).name} {slices} ... ",
                        end="",
                        flush=True,
                    )
...
def depthwise_conv2d_test():
    def _numpy_depthwise_conv2d(
        input_n, kernel_n, STRIDE=1, DILATION=1, padding="same", dtype=np.float32
    ):
            ...
            for oc in range(OC):
                for oh in range(OH):
                    for ow in range(OW):
                        ...
                        v = (
                            input_n[n, oc][..., I_idxs[0], I_idxs[1]]
                            * kernel_n[oc][..., K_idxs[0], K_idxs[1]]
                        ).sum()
                        ...
                        for ic in [1, 4]:
                            for ih, iw in zip(*[[4, 16]] * 2):
                                ...
                                for dtype in [np.int16, np.float16, np.float32]:
                                    ...
                                    print(
                                        f"depthwise_conv2d: {input_shape},{kernel_shape},{padding},{stride},{dilation},{np.dtype(dtype).name} ... ",
                                        end="",
                                        flush=True,
                                    )
                                    ...
                                    conved_n = _numpy_depthwise_conv2d(
                                        input_n,
                                        kernel_n,
                                        STRIDE=stride,
                                        DILATION=dilation,
                                    ...
                                    if not all(
                                        np.ndarray.flatten(conved_t.np() == conved_n)
                                    ):
                                        raise Exception(f"data is not equal")
...
def warp_affine_test():
    for dtype in HType.get_np_scalar_types():
        ...
        input_t = Tensor(
            [H, W, 2], dtype, initializer=InitCoords2DArange(0, H - 1, 0, W - 1)
        ).sum((-1,))

        affine_t = Tensor.from_value([[1, 0, 0], [0, 1, 0]], dtype)
...
def remap_np_affine_test():
    for dtype in HType.get_np_scalar_types():
        ...
        print(
            f"remap_np_affine: [{H},{W}] {np.dtype(dtype).name} ... ",
            end="",
            flush=True,
        )
        ...
        affine_n = np.array([[1, 0, 0], [0, 1, 0]], dtype)
    ...
    for _ in range(3):
        for shape_len in range(3, 5):
            for dtype in HType.get_np_scalar_types():
                ...
                print(
                    f"tile: {shape} {tiles} {np.dtype(dtype).name} ... ",
                    end="",
                    flush=True,
                )
    ...
    for _ in range(3):
        for shape_len in range(1, 4):
            for dtype in HType.get_np_scalar_types():
                ...
                print(
                    f"stack: {shape}*{stack_count} axis:{axis} {np.dtype(dtype).name} ... ",
                    end="",
                    flush=True,
                )

                vals_n = [
                    np.random.randint(2**8, size=shape).astype(dtype)
            ...
            if dtype != np.bool_:
                for shape_len in range(2, 5):
                    ...
                    reduction_axes = tuple(
                        reduction_axes[: np.random.randint(shape_len + 1)]
                    )
                    ...
                    print(
                        f"reduce {op_type}: {shape} {np.dtype(dtype).name} axes={reduction_axes} keepdims={keepdims} ... ",
                        end="",
                        flush=True,
                    )
                    ...
                    if dtype in [np.float16, np.float32]:
                        ...
                        value_n = np.random.randint(
                            max(1, int(np.iinfo(dtype).max / np.prod(shape))),
                            size=shape,
                            dtype=dtype,
                        )
...
def InitRandomUniform_test():
    for dtype in HType.get_np_scalar_types():
        for shape_len in range(1, 5):
            ...
            print(
                f"InitRandomUniform: {shape} {np.dtype(dtype).name} ... ",
                end="",
                flush=True,
            )
...
def InitCoords2DArange_test():
    for dtype in HType.get_np_scalar_types():
        for shape_len in range(2, 5):
            ...
            print(
                f"InitCoords2DArange: {shape} {np.dtype(dtype).name} ... ",
                end="",
                flush=True,
            )
            ...
            Tensor(
                shape,
                dtype,
                initializer=InitCoords2DArange(h_start, h_stop, w_start, w_stop),
            ).np()
...
def concat_test():
    for shape_len in range(2, 5):
        for dtype in HType.get_np_scalar_types():
            ...
            shapes = tuple(
                tuple(
                    dim if i != axis else np.random.randint(8) + 1
            ...
            print(
                f"concat: {shapes} axis={axis} {np.dtype(dtype).name} ... ",
                end="",
                flush=True,
            )

            V_n = [
                np.random.randint(2**8, size=shape).astype(dtype) for shape in shapes
            ]
...
def matmul_test():
    for _ in range(100):
        for dtype in [np.float32]:
            ...
            print(
                f"matmul: {A_shape} {B_shape} {np.dtype(dtype).name} ... ",
                end="",
                flush=True,
            )
        ...
        for shape_len in range(5, 1, -1):
            for dtype in HType.get_np_scalar_types():
                while True:
                    ...
                    print(
                        f"slice: {shape} {np.dtype(dtype).name} {slices} ... ",
                        end="",
                        flush=True,
                    )
        ...
        for dtype in HType.get_np_scalar_types():
            if dtype != np.bool_:
                ...
                for shape_len in shape_gen:
                    ...
                    print(
                        f"any_wise: {a_shape} {str(op_type)} {b_shape}:{str(np.dtype(dtype).name)} ...",
                        end="",
                        flush=True,
                    )
