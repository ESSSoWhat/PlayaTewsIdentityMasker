...
class HKernel:
    ...
    def define_tensor_type(name, dtype: np.dtype):
        ...
        if dtype == np.float16:
            ...
            out += [
                f"#define {name_upper}_TYPE16 {HKernel.np_dtype_to_cl(np.float32)}16"
            ]

            out += [
                f"#define {name_upper}_GLOBAL_LOAD(offset)   vload_half  (0, (const __global half*) (&{name_upper}_PTR_NAME[(offset)]) )"
            ]
            out += [
                f"#define {name_upper}_GLOBAL_LOAD2(offset)  vload_half2 (0, (const __global half*) (&{name_upper}_PTR_NAME[(offset)]) )"
            ]
            out += [
                f"#define {name_upper}_GLOBAL_LOAD3(offset)  vload_half3 (0, (const __global half*) (&{name_upper}_PTR_NAME[(offset)]) )"
            ]
            out += [
                f"#define {name_upper}_GLOBAL_LOAD4(offset)  vload_half4 (0, (const __global half*) (&{name_upper}_PTR_NAME[(offset)]) )"
            ]
            out += [
                f"#define {name_upper}_GLOBAL_LOAD8(offset)  vload_half8 (0, (const __global half*) (&{name_upper}_PTR_NAME[(offset)]) )"
            ]
            out += [
                f"#define {name_upper}_GLOBAL_LOAD16(offset) vload_half16(0, (const __global half*) (&{name_upper}_PTR_NAME[(offset)]) )"
            ]

            out += [
                f"#define {name_upper}_GLOBAL_STORE(offset,value)   vstore_half  ( (value), 0, (__global half*) (&{name_upper}_PTR_NAME[(offset)]) )"
            ]
            out += [
                f"#define {name_upper}_GLOBAL_STORE2(offset,value)  vstore_half2 ( (value), 0, (__global half*) (&{name_upper}_PTR_NAME[(offset)]) )"
            ]
            out += [
                f"#define {name_upper}_GLOBAL_STORE3(offset,value)  vstore_half3 ( (value), 0, (__global half*) (&{name_upper}_PTR_NAME[(offset)]) )"
            ]
            out += [
                f"#define {name_upper}_GLOBAL_STORE4(offset,value)  vstore_half4 ( (value), 0, (__global half*) (&{name_upper}_PTR_NAME[(offset)]) )"
            ]
            out += [
                f"#define {name_upper}_GLOBAL_STORE8(offset,value)  vstore_half8 ( (value), 0, (__global half*) (&{name_upper}_PTR_NAME[(offset)]) )"
            ]
            out += [
                f"#define {name_upper}_GLOBAL_STORE16(offset,value) vstore_half16( (value), 0, (__global half*) (&{name_upper}_PTR_NAME[(offset)]) )"
            ]
            ...
            out += [
                f"#define {name_upper}_PTR_TYPE16 {HKernel.np_dtype_to_cl(dtype)}16"
            ]
            ...
            out += [
                f"#define {name_upper}_GLOBAL_LOAD(offset)   {name_upper}_PTR_NAME[(offset)]"
            ]
            out += [
                f"#define {name_upper}_GLOBAL_LOAD2(offset)  {name_upper}_PTR_NAME[(offset)]"
            ]
            out += [
                f"#define {name_upper}_GLOBAL_LOAD3(offset)  {name_upper}_PTR_NAME[(offset)]"
            ]
            out += [
                f"#define {name_upper}_GLOBAL_LOAD4(offset)  {name_upper}_PTR_NAME[(offset)]"
            ]
            out += [
                f"#define {name_upper}_GLOBAL_LOAD8(offset)  {name_upper}_PTR_NAME[(offset)]"
            ]
            out += [
                f"#define {name_upper}_GLOBAL_LOAD16(offset) {name_upper}_PTR_NAME[(offset)]"
            ]

            out += [
                f"#define {name_upper}_GLOBAL_STORE(offset,value) {name_upper}_PTR_NAME[(offset)] = (value)"
            ]
            out += [
                f"#define {name_upper}_GLOBAL_STORE2(offset,value) {name_upper}_PTR_NAME[(offset)] = (value)"
            ]
            out += [
                f"#define {name_upper}_GLOBAL_STORE3(offset,value) {name_upper}_PTR_NAME[(offset)] = (value)"
            ]
            out += [
                f"#define {name_upper}_GLOBAL_STORE4(offset,value) {name_upper}_PTR_NAME[(offset)] = (value)"
            ]
            out += [
                f"#define {name_upper}_GLOBAL_STORE8(offset,value) {name_upper}_PTR_NAME[(offset)] = (value)"
            ]
            out += [
                f"#define {name_upper}_GLOBAL_STORE16(offset,value) {name_upper}_PTR_NAME[(offset)] = (value)"
            ]
    ...
    def define_ndim_idx(ndim):
        ...
        out = [
            f"#define NDIM{ndim}_IDX("
            + ",".join([f"t{i}" for i in range(ndim)] + [f"T{i}" for i in range(ndim)])
            + ") ("
            + "+".join(
                [
                    f"((size_t)(t{i}))" + "".join(f"*T{j}" for j in range(i + 1, ndim))
                    for i in range(ndim)
                ]
            )
            + ")"
        ...
        out += [
            f"#define NDIM{ndim}_IDX_MOD("
            + ",".join([f"t{i}" for i in range(ndim)] + [f"T{i}" for i in range(ndim)])
            + ") ("
            + "+".join(
                [
                    f"( (((size_t)(t{i}) % T{i}) + T{i}) % T{i} )    "
                    + "".join(f"*T{j}" for j in range(i + 1, ndim))
                    for i in range(ndim)
                ]
            )
            + ")"
    ...
    def define_tensor_shape(name, shape, axes_symbols=None):
        ...
        out += [
            f"#define {name_upper}_IDX({HKernel.axes_seq_enum(name, ndim)}) ("
            + "+".join(
                [
                    f"((size_t)({name_lower}{i}))"
                    + "".join(f"*{shape[j]}" for j in range(i + 1, ndim))
                    for i in range(ndim)
                ]
            )
            + ")"
        ...
        out += [
            f"#define {name_upper}_IDX_MOD({HKernel.axes_seq_enum(name, ndim)}) ("
            + "+".join(
                [
                    f"( (( (size_t)({name_lower}{i}) % {shape[i]} ) + {shape[i]}) % {shape[i]} )"
                    + "".join(f"*{shape[j]}" for j in range(i + 1, ndim))
                    for i in range(ndim)
                ]
            )
    ...
    def define_tensor(name, shape, dtype: np.dtype, axes_symbols=None):
        ...
        return "\n".join(
            [
                HKernel.define_tensor_type(name, dtype),
    ...
    def axes_order_enum(tensor_name, axes_order):
        ...
        return ",".join(
            [f"{name_lower}{axes_order[axis]}" for axis in range(len(axes_order))]
        )
    ...
    def axes_seq_enum(tensor_name, ndim, new_axis=None, zero_axes=None, suffix=None):
        ...
        if zero_axes is not None:
            axes = [
                "0" if axis in zero_axes else f"{name_lower}{axis}"
