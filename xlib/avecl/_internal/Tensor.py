...
class Tensor:
    ...
    def __init__(
        ...
        self._buffer = Buffer(
            ...
            on_initialize=lambda initializer=initializer, wself=weakref.ref(self): (
                initializer.initialize_tensor(wself())
    ...
    @staticmethod
    def from_value(
        value, dtype=None, device: Union[None, int, Device, DeviceInfo] = None
    ) -> "Tensor":
    ...
    def set(self, value: Union["Tensor", np.ndarray, int, float]) -> "Tensor":
        if isinstance(value, Tensor):
            if self.shape.size != value.shape.size:
                raise ValueError(
                    "Unable to set the data from other tensor: shape.size is not the same."
                )
    ...
    @property
    def T(self):
        return self.transpose(tuple(range(self.shape.ndim - 1, -1, -1)))
