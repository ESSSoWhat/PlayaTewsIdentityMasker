...
def any_wise(
    ...
    *args,
    ...
    op = SCacheton.get(
        _AnyWiseOp, shape_list, dtype_list, dim_wise_axis, dtype, op_text
    )
...
class _AnyWiseOp:
    def __init__(self, shape_list, dtype_list, dim_wise_axis, o_dtype, op_text: str):
        ...
        self.o_dtype = o_dtype = (
            ...
            else HType.get_most_weighted_dtype(dtype_list)
        )
        self.info = info = BroadcastInfo(
            [shape if shape is not None else AShape((1,)) for shape in shape_list]
        )
        ...
        for i, (t_shape, t_dtype) in enumerate(zip(shape_list, dtype_list)):
            ...
            if t_shape is not None:
                ...
                arg_defs.append(
                    f", __global const {t_name}_PTR_TYPE* {t_name}_PTR_NAME"
                )

                if dim_wise_axis is not None:
                    for i_elem in range(dim_wise_axis_size):
                        impls.append(
                            f"{t_name}_TYPE {t_name}_{i_elem} = {t_name}_GLOBAL_LOAD({t_name}_IDX_MOD({HKernel.axes_seq_enum('G', g_shape.ndim, new_axis=(f'{i_elem}', dim_wise_axis) )}));"
                        )
                else:
                    impls.append(
                        f"{t_name}_TYPE {t_name} = {t_name}_GLOBAL_LOAD({t_name}_IDX_MOD({HKernel.axes_seq_enum('G', g_shape.ndim)}));"
                    )
            else:
        ...
        if dim_wise_axis is not None:
            o_def = "\n".join(
                f"O_TYPE O_{i_elem};" for i_elem in range(dim_wise_axis_size)
            )
            o_store = "\n".join(
                f"O_GLOBAL_STORE(O_IDX({HKernel.axes_seq_enum('G', g_shape.ndim, new_axis=(f'{i_elem}', dim_wise_axis) )}), O_{i_elem});"
                for i_elem in range(dim_wise_axis_size)
            )
        else:
            o_def = "O_TYPE O;"
...
def add(a_t: Tensor, b_t: Tensor) -> Tensor:
    return any_wise("O=I0+I1", a_t, b_t)
...
def min_(a_t: Tensor, b_t: Tensor) -> Tensor:
    return any_wise("O=fmin( I0_TO_FLOATX(I0), I1_TO_FLOATX(I1) )", a_t, b_t)


def max_(a_t: Tensor, b_t: Tensor) -> Tensor:
    return any_wise("O=fmax( I0_TO_FLOATX(I0), I1_TO_FLOATX(I1) )", a_t, b_t)


def square(a_t: Tensor) -> Tensor:
    return any_wise("O=I0*I0", a_t)


def sqrt(a_t: Tensor) -> Tensor:
    return any_wise("O=sqrt(I0_TO_FLOATX(I0))", a_t)
