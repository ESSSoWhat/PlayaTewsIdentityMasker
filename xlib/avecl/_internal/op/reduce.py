...
from .any_wise import sqrt, square
from .slice_ import slice_
from .transpose import transpose


def reduce_mean(
    input_t: Tensor, axes=None, keepdims=False, output_t=None, is_add_to_output=False
) -> Tensor:
    """
    ...
    return reduce_op(
        ...
        keepdims=keepdims,
        output_t=output_t,
        is_add_to_output=is_add_to_output,
    )
...
def reduce_op(
    ...
    for stage, (shape, STAGE_COLS, STAGE_VALID_COLS) in enumerate(
        zip(
            op.forward_krn_shapes,
            op.forward_krn_stage_cols,
            op.forward_krn_stage_valid_cols,
        )
    ):
        input_t_inter.get_device().run_kernel(
            op.forward_krn,
            input_t_inter.get_buffer(),
            np.int64(op.COLS),
            np.int64(STAGE_COLS),
            np.int64(STAGE_VALID_COLS),
            global_shape=shape,
        )
    ...
    if op_type == "mean":
        ...
        input_t_inter.get_device().run_kernel(
            op.mean_div_forward_krn,
            input_t_inter.get_buffer(),
            np.int64(op.COLS),
            global_shape=(op.ROWS,),
        )
...
class _ReduceOp:
    def __init__(
        self, op_type, i_shape: AShape, i_dtype: np.dtype, axes: AAxes, keepdims=False
    ):
        ...
        self.intermediate_shape = TransposeInfo(
            i_shape, self.intermediate_transpose_axes
        ).o_shape

        # slices argument to fetch processed tensor from zero indexes
        self.inter_slices = (slice(None, None, None),) * info.o_axes.ndim + (
            0,
        ) * info.reduction_axes.ndim
        ...
        self.forward_krn_shapes = [
            (ROWS * math.ceil(COLS / (2 ** (stage + 1))),) for stage in range(n_stages)
        ]
        self.forward_krn_stage_cols = [
            math.ceil(COLS / (2 ** (stage + 1))) for stage in range(n_stages)
        ]
        self.forward_krn_stage_valid_cols = [
            math.ceil(COLS / (2**stage)) for stage in range(n_stages)
        ]
...
"""
        ...
        self.mean_div_forward_krn = Kernel(
            f"""
{HKernel.define_tensor('I', (1,), i_dtype)}
