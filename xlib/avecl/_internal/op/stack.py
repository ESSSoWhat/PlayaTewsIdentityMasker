...
def stack(
    tensor_list: List[Tensor], axis, dtype=None, output_t=None, is_add_to_output=False
):
    """
    Stack operator.
    ...
    for i, krn in enumerate(op.forward_krns):
        device.run_kernel(
            krn, output_t.get_buffer(), tensor_list[i].get_buffer(), np.int64(i)
        )
