...
def slice_set(input_t: Tensor, slices, value) -> Tensor:
    ...
    if HType.is_scalar_type(value):
        ...
        value = Tensor.from_value(
            value, dtype=input_t.dtype, device=input_t.get_device()
        )
        v_shape = value.shape
        v_dtype = value.dtype
        v_scalar = None
    ...
    if v_scalar is not None:
        ...
        input_t.get_device().run_kernel(
            op.forward_krn, input_t.get_buffer(), value.get_buffer()
        )
    ...
    def __init__(
        ...
        if v_scalar is None:
            if v_shape.ndim > i_shape.ndim:
                raise ValueError(
                    f"v_shape.ndim {v_shape.ndim} cannot be larger than i_shape.ndim {i_shape.ndim}"
                )
