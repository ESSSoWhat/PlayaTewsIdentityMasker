...
def rct(
    target_t: Tensor,
    source_t: Tensor,
    target_mask_t: Tensor = None,
    source_mask_t: Tensor = None,
    mask_cutoff=0.5,
) -> Tensor:
    """
    ...
    if target_mask_t is not None:
        target_stat_t = any_wise(
            "O = I0*(I1 >= I2)", target_stat_t, target_mask_t, np.float32(mask_cutoff)
        )

    source_stat_t = source_t
    if source_mask_t is not None:
        source_stat_t = any_wise(
            "O = I0*(I1 >= I2)", source_stat_t, source_mask_t, np.float32(mask_cutoff)
        )
