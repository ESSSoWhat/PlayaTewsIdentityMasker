...
class AAxes(Iterable):
                ...
                for x in axes:
                    ...
                    if x < 0:
                        if shape_ndim is None:
                            raise ValueError(
                                f"Incorrent value {x} in axes {axes}, or provide shape_ndim"
                            )
                        x = shape_ndim + x
    ...
    def inversed(self) -> "AAxes":
        ...
        if self.is_none_axes():
            raise Exception(
                f"none-axes does not support inversed(). Handle none-axes by calling .is_none_axes()"
            )
        ...
        if self._inversed is None:
            ...
            for i in range(self.ndim):
                ...
                if axis is None:
                    raise Exception(
                        f"axes {self.axes} are inconsistent to do inverse order."
                    )
                t.append(axis)
    ...
    def __iter__(self):
        if self.is_none_axes():
            raise Exception(
                f"none-axes does not support iteration. Handle none-axes by calling .is_none_axes()"
            )
    ...
    def __getitem__(self, key):
        if self.is_none_axes():
            raise Exception(
                f"none-axes does not support indexing. Handle none-axes by calling .is_none_axes()"
            )
