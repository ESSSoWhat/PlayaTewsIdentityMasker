...
class DlgChoice:
    def __init__(
        self, short_name: str = None, row_def: str = None, on_choose: Callable = None
    ):
        if len(short_name) == 0:
    ...
    def get_row_def(self) -> Union[str, None]:
        return self._row_def

    def get_on_choose(self) -> Callable:
...
class DlgNumber(Dlg):
    def __init__(
        ...
        if (
            clip_min_value is not None
            and clip_max_value is not None
            and clip_min_value > clip_max_value
        ):
    ...
    def on_user_input(self, s: str) -> bool:
        ...
        if result == EDlgMode.UNHANDLED:
            ...
            for s in multi_s:
                x = [
                    i
                    for i, short_name in enumerate(self._short_names)
                    if s.strip() == short_name
                ]
                if len(x) == 0:
                    # No short name match
                    return EDlgMode.WRONG_INPUT
