...
class Faceset:
    ...
    def optimize(self, verbose=True):
        ...
        tmp_path = self._path.parent / (
            self._path.stem + "_optimizing" + self._path.suffix
        )
        ...
        self._group_copy(
            tmp_fs._UImage_image_data_grp, self._UImage_image_data_grp, verbose=verbose
        )
        tmp_fs.close()
    ...
    def _group_copy(self, group_dst: h5py.Group, group_src: h5py.Group, verbose=True):
        for key, value in lib_con.progress_bar_iterator(
            group_src.items(),
            desc=f"Copying {group_src.name} -> {group_dst.name}",
    ...
    def _group_read_bytes(
        self, group: h5py.Group, key: str, check_key=True
    ) -> Union[bytes, None]:
    ...
    def _group_write_bytes(
        self, group: h5py.Group, key: str, data: bytes, update_existing=True
    ...
    def add_UFaceMark(self, ufacemark_or_list: UFaceMark, update_existing=True):
        ...
        for ufm in ufacemark_or_list:
            self._group_write_bytes(
                self._UFaceMark_grp,
                ufm.get_uuid().hex(),
                pickle.dumps(ufm.dump_state()),
    ...
    def get_all_UFaceMark(self) -> List[UFaceMark]:
        return [
            UFaceMark.from_state(
                pickle.loads(
                    self._group_read_bytes(self._UFaceMark_grp, key, check_key=False)
                )
            )
    ...
    def add_UImage(
        ...
        for uimage in uimage_or_list:
            ...
            ret, data_bytes = cv2.imencode(
                f".{format}", uimage.get_image(), imencode_args
            )
            if not ret:
            ...
            self._group_write_bytes(
                self._UImage_grp,
                key,
                pickle.dumps(uimage.dump_state(exclude_image=True)),
                update_existing=update_existing,
            )
            d = self._group_write_bytes(
                self._UImage_image_data_grp,
                key,
                data_bytes.data,
                update_existing=update_existing,
            )
    ...
    def _get_UImage_by_key(self, key, check_key=True) -> Union[UImage, None]:
        ...
        image_data = self._group_read_bytes(
            self._UImage_image_data_grp, key, check_key=check_key
        )
    ...
    def add_UPerson(self, uperson_or_list: UPerson, update_existing=True):
        ...
        for uperson in uperson_or_list:
            self._group_write_bytes(
                self._UPerson_grp,
                uperson.get_uuid().hex(),
                pickle.dumps(uperson.dump_state()),
                update_existing=update_existing,
            )
    ...
    def get_all_UPerson(self) -> List[UPerson]:
        return [
            UPerson.from_state(
                pickle.loads(
                    self._group_read_bytes(self._UPerson_grp, key, check_key=False)
                )
            )
