...
class FaceWarper:
    def __init__(
        ...
        rnd_state.set_state(
            ...
            else np.random.RandomState().get_state()
        )

        self._align_rot_deg = (
            rnd_state.uniform(*align_rot_deg)
            if isinstance(align_rot_deg, Iterable)
            else align_rot_deg
        )
        self._align_scale = (
            rnd_state.uniform(*align_scale)
            if isinstance(align_scale, Iterable)
            else align_scale
        )
        self._align_tx = (
            rnd_state.uniform(*align_tx) if isinstance(align_tx, Iterable) else align_tx
        )
        self._align_ty = (
            rnd_state.uniform(*align_ty) if isinstance(align_ty, Iterable) else align_ty
        )
        self._rw_grid_cell_count = (
            rnd_state.randint(*rw_grid_cell_count)
            if isinstance(rw_grid_cell_count, Iterable)
            else rw_grid_cell_count
        )
        self._rw_grid_rot_deg = (
            rnd_state.uniform(*rw_grid_rot_deg)
            if isinstance(rw_grid_rot_deg, Iterable)
            else rw_grid_rot_deg
        )
        self._rw_grid_scale = (
            rnd_state.uniform(*rw_grid_scale)
            if isinstance(rw_grid_scale, Iterable)
            else rw_grid_scale
        )
        self._rw_grid_tx = (
            rnd_state.uniform(*rw_grid_tx)
            if isinstance(rw_grid_tx, Iterable)
            else rw_grid_tx
        )
        self._rw_grid_ty = (
            rnd_state.uniform(*rw_grid_ty)
            if isinstance(rw_grid_ty, Iterable)
            else rw_grid_ty
        )

        self._warp_rnd_mat = Affine2DUniMat.from_transformation(
            0.5,
            0.5,
            self._rw_grid_rot_deg,
            1.0 + self._rw_grid_scale,
            self._rw_grid_tx,
            self._rw_grid_ty,
        )
        self._align_rnd_mat = Affine2DUniMat.from_transformation(
            0.5,
            0.5,
            self._align_rot_deg,
            1.0 + self._align_scale,
            self._align_tx,
            self._align_ty,
        )
    ...
    def transform(
        self, img: np.ndarray, out_res: int, random_warp: bool = True
    ) -> np.ndarray:
        """
        transform an image.
        ...
        if data is None:
            ...
            self._cached[key] = data = self._gen(
                H, W, random_warp, out_res, rnd_state=rnd_state
            )
    ...
    def _gen(self, H, W, random_warp, out_res, rnd_state):
        ...
        if random_warp:
            ...
            face_warp_grid = FaceWarper._gen_random_warp_uni_grid_diff(
                out_res, self._rw_grid_cell_count, 0.12, rnd_state
            )
            ...
            image_grid += cv2.warpAffine(
                face_warp_grid,
                face_warp_grid_uni_mat.to_exact_mat(out_res, out_res, W, H),
                (W, H),
        ...
        img_to_face_rnd_mat = (
            (self._face_to_img_uni_mat * self._align_rnd_mat)
            .invert()
            .to_exact_mat(W, H, out_res, out_res)
        )
    ...
    def _gen_random_warp_uni_grid_diff(
        size: int, cell_count, cell_mod, rnd_state
    ) -> np.ndarray:
        """
        ...
        grid[1:-1, 1:-1, 0:2] += rnd_state.uniform(
            low=-cell_size * cell_mod,
            high=cell_size * cell_mod,
            size=(cell_count - 2, cell_count - 2, 2),
        )
        grid = cv2.resize(grid, (size, size), interpolation=cv2.INTER_CUBIC).astype(
            np.float32
        )
