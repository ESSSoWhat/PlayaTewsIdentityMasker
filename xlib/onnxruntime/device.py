...
class ORTDeviceInfo:
    ...
    def __eq__(self, other):
        if (
            ...
            and isinstance(self, ORTDeviceInfo)
            and isinstance(other, ORTDeviceInfo)
        ):
    ...
    if int(os.environ.get("ORT_DEVICES_INITIALIZED", 0)) == 0:
        ...
        if not lib_appargs.get_arg_bool("NO_CUDA") and "CUDAExecutionProvider" in prs:
            ...
            try:
                ...
                if (
                    cuda.cuInit(0) == 0
                    and cuda.cuDeviceGetCount(ctypes.byref(nGpus)) == 0
                ):
                    for i in range(nGpus.value):
                        if (
                            cuda.cuDeviceGet(ctypes.byref(device), i) != 0
                            or cuda.cuDeviceGetName(
                                ctypes.c_char_p(name), len(name), device
                            )
                            != 0
                            or cuda.cuDeviceComputeCapability(
            ...
            if dxgi_factory is not None:
                for i in itertools.count():
                    ...
                    if adapter is not None:
                        ...
                        if (
                            desc.Flags
                            != lib_dxgi.DXGI_ADAPTER_FLAG.DXGI_ADAPTER_FLAG_SOFTWARE
                            and not (desc.VendorId == 0x1414 and desc.DeviceId == 0x8C)
                        ):
                            devices.append(
                                {
                                    "index": i,
