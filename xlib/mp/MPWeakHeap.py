...
class MPWeakHeap:
    ...
    def __init__(self, size_mb: int):
        ...
        fmv.seek(self._ring_head_block_offset), fmv.write_fmt(
            "q", self._first_block_offset
        )

        # Entire block
        fmv.seek(self._first_block_offset)
        fmv.write_fmt("qq", self._heap_size - self._first_block_offset, 0), fmv.write(
            uuid.uuid4().bytes
        )
        ...
        while True:
            ...
            if data_size <= block_free_size:
                ...
                if block_remain_size >= block_header_size:
                    ...
                    fmv.seek(next_block_offset), fmv.write_fmt(
                        "qq", block_remain_size, 0
                    ), fmv.write(uuid.uuid4().bytes)
                else:
                ...
                fmv.seek(cur_block_offset), fmv.write_fmt(
                    "qq", block_new_size, data_size
                ), fmv.write(uid)
                ...
                if is_last_block:
                    if is_first_block:
                        lock.release()
                        raise Exception(
                            f"Not enough space in MPWeakHeap to allocate {data_size}"
                        )
