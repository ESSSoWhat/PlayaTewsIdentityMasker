...
class MPSPSCMRRingData:
    ...
    def __init__(self, table_size, heap_size_mb, multi_producer: bool = False):
        ...
        self._shared_mem = MPSharedMemory(
            8 + 8 + table_size * table_item_size + heap_size
        )
    ...
    def get_write_id(self) -> int:
        return self._mv_ids[0]

    def get_read_id(self) -> int:
        return self._mv_ids[1]
    ...
    def write(self, data: Union[bytes, bytearray]):
        ...
        fmv.seek(self._table_offset + (wid % self._table_size) * self._table_item_size)
        (wid_heap_offset, wid_data_size), _ = fmv.read_fmt("QQ"), fmv.read(
            self._sizeof_uuid
        )
    ...
    def get_by_id(self, id) -> Union[bytearray, None]:
        ...
        (rid_heap_offset, rid_data_size), rid_uuid = fmv.read_fmt("QQ"), fmv.read(
            sizeof_uuid
        )
