...
class MPWorker:
    def __init__(self, sub_args: List = None, process_count: int = None):
        """
        ...
        for i in range(process_count):
            ...
            p = multiprocessing.Process(
                target=self._sub_process,
                args=(i, process_count, sub_pipe, sub_args),
                daemon=True,
            )
            p.start()
        ...
        threading.Thread(
            target=_host_thread_proc, args=(weakref.ref(self),), daemon=True
        ).start()
    ...
    def _host_process_messages(self, timeout: float = 0) -> bool:
        ...
        for process_id, pipe in enumerate(self._pipes):
            try:
                ...
                print(
                    f"Unexpected error processing messages from process {process_id}: {e}"
                )
                continue
    ...
    def _send_msg(self, name, *args, process_id=-1, **kwargs):
        ...
        try:
            ...
            # Handle other unexpected errors
            print(
                f"Unexpected error sending message '{name}' to process {process_id}: {e}"
            )
                ...
                while True:
                    ...
                    if name == "__stop":
                        ...
                        try:
                            ...
                            print(
                                f"Error during handling host message {name} : {traceback.format_exc()}"
                            )
