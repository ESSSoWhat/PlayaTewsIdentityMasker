...
class _PathBase:
    ...
    def _set_paths(self, path_or_list, block_event=False):
        if isinstance(path_or_list, Iterable) and not isinstance(path_or_list, str):
...
class Paths:
    ...
    class Config:
        ...
        def get_type(self) -> "Paths.Config.Type":
            return self._type

        def is_save(self) -> bool:
            return self._is_save

        def get_caption(self) -> Union[str, None]:
            return self._caption

        def get_suffixes(self) -> Union[List[str], None]:
            return self._suffixes

        def get_directory_path(self) -> Union[Path, None]:
        ...
        def ExistingFile(is_save=False, caption=None, suffixes=None):
            return Paths.Config(
                Paths.Config.Type.EXISTING_FILE, is_save, caption, suffixes
            )

        @staticmethod
        def ExistingFiles(caption=None, suffixes=None):
            return Paths.Config(
                Paths.Config.Type.EXISTING_FILES, False, caption, suffixes
            )

        @staticmethod
        def Directory(caption=None, directory_path=None):
            return Paths.Config(
                Paths.Config.Type.DIRECTORY,
                False,
                caption,
                None,
