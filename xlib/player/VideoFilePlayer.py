...
class VideoFilePlayer(FramePlayer):
    ...
    def __init__(self, filepath):
        ...
        if not filepath.suffix in VideoFilePlayer.SUPPORTED_VIDEO_FILE_SUFFIXES:
            raise Exception(
                f"Supported video files: {VideoFilePlayer.SUPPORTED_VIDEO_FILE_SUFFIXES}"
            )
        ...
        stream_frame_count = round(
            (float(stream_duration) - float(stream_start_time)) / (1.0 / stream_fps)
        )
    ...
    def _ffmpeg_restart(self, start_frame_number=0):
        ...
        if _target_width == 0:
            ...
            _height = self._ffmpeg_height = int(
                _target_width / (self._stream_width / self._stream_height)
            )
        ...
        args += ["-i", str(self._filepath), "-s", f"{_width}:{_height}"]
        ...
        args += [
            ...
            "bgr24",
            "-map",
            f"0:v:{self._stream_idx}",
            "pipe:",
        ]

        self._ffmpeg_proc = ffmpeg_proc = lib_ffmpeg.run(
            args, pipe_stdout=True, pipe_stderr=True
        )
        self._ffmpeg_proc_stderr_lines = None
        if ffmpeg_proc is not None:
            self._ffmpeg_proc_stderr_lines = lib_io.IOThreadLinesReader(
                ffmpeg_proc.stderr, max_lines=5
            )
            return True
        return False
    ...
    def _ffmpeg_next_frame(self, frames_idx_offset=1):
        ...
        while frames_idx_offset != 0:
            frame_buffer = self._ffmpeg_proc.stdout.read(
                self._ffmpeg_height * self._ffmpeg_width * 3
            )

            if len(frame_buffer) == 0:
                # unpredicted end reached
                err = "\r\n".join(
                    self._ffmpeg_proc_stderr_lines.get_lines(till_eof=True)[-5:]
                )
                self._ffmpeg_stop()
                return None, err
            frames_idx_offset -= 1
    ...
    def _on_get_frame(self, idx) -> Tuple[np.ndarray, str]:
        ...
        if self._ffmpeg_proc is None or frame_diff <= 0 or frame_diff >= 100:
            self._ffmpeg_need_restart = True
