...
class ImageSequencePlayer(FramePlayer):
    ...
    def __init__(
        ...
        images_paths = lib_path.get_files_paths(
            dir_path, ImageSequencePlayer.SUPPORTED_IMAGE_SEQUENCE_SUFFIXES
        )
        if len(images_paths) == 0:
            raise Exception(
                f"Images with extensions {ImageSequencePlayer.SUPPORTED_IMAGE_SEQUENCE_SUFFIXES} are not found in directory: /{dir_path.name}/"
            )
