...
class Affine2DMat(np.ndarray):
    ...
    def __rmul__(self, other) -> "Affine2DMat":
        if isinstance(other, Affine2DMat):
            return Affine2DMat(
                np.matmul(
                    np.concatenate([other, [[0, 0, 1]]], 0),
                    np.concatenate([self, [[0, 0, 1]]], 0),
                )[:2]
            )
    ...
    def from_transformation(
        cx: float, cy: float, rot_deg: float, scale: float, tx: float, ty: float
    ) -> "Affine2DMat":
        """
    ...
    def from_3_pairs(src_pts, dst_pts) -> "Affine2DMat":
        ...
        """
        return Affine2DMat(
            cv2.getAffineTransform(np.float32(src_pts), np.float32(dst_pts))
        )
    ...
    def invert(self):
        ...
        return Affine2DMat(((a, b, c), (d, e, f)))
    ...
    def transform_points(self, points):
        ...
        return (
            np.matmul(np.concatenate([self, [[0, 0, 1]]], 0), points.T)
            .T[:, :2]
            .astype(dtype)
        )
...
class Affine2DUniMat(Affine2DMat):
    ...
    @staticmethod
    def identity():
        return Affine2DMat.identity().as_uni_mat()

    @staticmethod
    def umeyama(src, dst, estimate_scale=True):
        return Affine2DMat.umeyama(src, dst, estimate_scale=estimate_scale).as_uni_mat()
    ...
    def from_transformation(
        ...
        tx, ty     additional translate x,y
        """
        return Affine2DMat.from_transformation(
            cx, cy, rot_deg, scale, tx, ty
        ).as_uni_mat()

    @staticmethod
    def from_3_pairs(src_pts, dst_pts) -> "Affine2DUniMat":
        return Affine2DMat.from_3_pairs(src_pts, dst_pts).as_uni_mat()

    def invert(self) -> "Affine2DUniMat":
        return super().invert().as_uni_mat()
    ...
    def to_exact_mat(self, sw: float, sh: float, tw: float, th: float) -> "Affine2DMat":
        ...
        """
        return Affine2DMat.from_3_pairs(
            [[0, 0], [sw, 0], [0, sh]],
