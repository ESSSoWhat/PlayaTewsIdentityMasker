...
class QXImageSequenceDB:
    ...
    def _get(dir_path: Path, fps, color=None):
        ...
        if result is None:
            ...
            result = QXImageSequenceDB.cached[key] = QXImageSequence(
                frames=[
                    QXImage_from_file(image_path, color) for image_path in image_paths
                ],
                fps=fps,
            )
        return result

    def icon_loading(color=None) -> QXImageSequence:
        return QXImageSequenceDB._get(
            Path(__file__).parent / "sequences" / "icon_loading", fps=30, color=color
        )
