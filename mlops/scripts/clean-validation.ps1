# Clean MLOps Validation Script (PowerShell)
...
function Test-Service {
    ...
    try {
        ...
        if ($response.StatusCode -eq 200) {
            Write-Host "PASS: $ServiceName responding" -ForegroundColor Green
            return $true
        } else {
            Write-Host "FAIL: $ServiceName not responding (Status: $($response.StatusCode))" -ForegroundColor Red
            return $false
        }
    }
    catch {
        Write-Host "FAIL: $ServiceName not responding" -ForegroundColor Red
        return $false
    }
}
...
if (Get-Command kubectl -ErrorAction SilentlyContinue) {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "PASS: MLOps namespace exists" -ForegroundColor Green
            
            $pods = kubectl get pods -n mlops 2>$null
            if ($pods -match "Running") {
                Write-Host "PASS: Kubernetes pods running" -ForegroundColor Green
            } else {
                Write-Host "FAIL: Kubernetes pods not running" -ForegroundColor Red
            }
        } else {
            Write-Host "WARN: MLOps namespace not found" -ForegroundColor Yellow
        }
    }
    catch {
        Write-Host "WARN: Kubernetes checks failed" -ForegroundColor Yellow
    }
} else {
    Write-Host "WARN: kubectl not available - skipping Kubernetes checks" -ForegroundColor Yellow
}
...
if (Get-Command aws -ErrorAction SilentlyContinue) {
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "PASS: AWS CLI configured" -ForegroundColor Green
        } else {
            Write-Host "WARN: AWS CLI not authenticated" -ForegroundColor Yellow
        }
    }
    catch {
        Write-Host "WARN: AWS CLI not authenticated" -ForegroundColor Yellow
    ...
    Write-Host "WARN: AWS CLI not available" -ForegroundColor Yellow
}
...
if (Get-Command az -ErrorAction SilentlyContinue) {
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "PASS: Azure CLI configured" -ForegroundColor Green
        } else {
            Write-Host "WARN: Azure CLI not authenticated" -ForegroundColor Yellow
        }
    }
    catch {
        Write-Host "WARN: Azure CLI not authenticated" -ForegroundColor Yellow
    ...
    Write-Host "WARN: Azure CLI not available" -ForegroundColor Yellow
}
...
if (Get-Command gcloud -ErrorAction SilentlyContinue) {
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "PASS: Google Cloud CLI configured" -ForegroundColor Green
        } else {
            Write-Host "WARN: Google Cloud CLI not authenticated" -ForegroundColor Yellow
        }
    }
    catch {
        Write-Host "WARN: Google Cloud CLI not authenticated" -ForegroundColor Yellow
    ...
    Write-Host "WARN: Google Cloud CLI not available" -ForegroundColor Yellow
}

# 6. Monitoring and Maintenance Validation
Write-Host "" -ForegroundColor White
...
try {
    ...
    if ($metrics.data.result.Count -gt 0) {
        Write-Host "PASS: Monitoring metrics being collected" -ForegroundColor Green
        $LocalPassed++
    } else {
        Write-Host "FAIL: Monitoring metrics not being collected" -ForegroundColor Red
    }
}
catch {
    Write-Host "FAIL: Monitoring metrics not being collected" -ForegroundColor Red
...
if (Test-Path $backupDir) {
    ...
    if (Test-Path $todayBackup) {
        Write-Host "PASS: Today's backup exists" -ForegroundColor Green
    } else {
        Write-Host "WARN: Today's backup not found" -ForegroundColor Yellow
    }
} else {
    Write-Host "WARN: Backup directory not found" -ForegroundColor Yellow
}
...
Write-Host "Local Development: $LocalPassed of $LocalChecks checks passed ($LocalPercent percent)" -ForegroundColor White
Write-Host "Staging: $StagingPassed of $StagingChecks checks passed ($StagingPercent percent)" -ForegroundColor White
Write-Host "Production: $ProductionPassed of $ProductionChecks checks passed ($ProductionPercent percent)" -ForegroundColor White
...
Write-Host "" -ForegroundColor White
Write-Host "Overall Status: $TotalPassed of $TotalChecks checks passed ($OverallPercent percent)" -ForegroundColor White
...
if ($OverallPercent -ge 90) {
    ...
    Write-Host "FAIR: Several issues need fixing" -ForegroundColor Yellow
} else {
