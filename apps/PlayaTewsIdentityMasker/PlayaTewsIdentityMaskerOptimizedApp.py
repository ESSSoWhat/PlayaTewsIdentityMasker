...
class QLiveSwapOptimized(qtx.QXWidget):
    def __init__(self, userdata_path: Path, settings_dirpath: Path):
        ...
        self.q_face_animator = QOptimizedFaceAnimator(
        ...
        self.q_grouped_input_sources = QGroupedInputSources(
            self.q_file_source, self.q_camera_source
        )
        self.q_grouped_face_detection = QGroupedFaceDetection(
        ...
        try:
            self.q_obs_style_ui = QOptimizedOBSStyleUI(
                self.stream_output,
                userdata_path,
                face_swap_components,
                viewers_components,
            ...
            self.q_obs_style_ui = qtx.QXLabel(
                text="Optimized OBS-Style UI not available\nUsing Traditional Interface"
            )
...
class QDFLOptimizedAppWindow(qtx.QXWindow):
    ...
    def _on_backup_layout(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Backup Failed", f"Failed to create UI layout backup:\n{str(e)}"
            )
    ...
    def _on_restore_layout(self):
        ...
        try:
            ...
            if not backups:
                ...
                QMessageBox.information(
                    self, "No Backups", "No UI layout backups found."
                )
                return
            ...
            QMessageBox.critical(
                self, "Restore Failed", f"Failed to restore UI layout:\n{str(e)}"
            )
    ...
    def _on_manage_backups(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"Failed to open backup manager:\n{str(e)}"
            )
...
class PlayaTewsIdentityMaskerOptimizedApp(qtx.QXMainApplication):
    def __init__(self, userdata_path):
        ...
        super().__init__(
            app_name="PlayaTewsIdentityMaskerOptimized",
        ...
        self.q_main_window = QDFLOptimizedAppWindow(
