...
class FaceSwapInsightWorker(BackendWorker):
    ...
    def on_start(
        ...
        cs.device.set_choices(
            InsightFaceSwap.get_available_devices(),
    ...
    def update_faces(self):
        ...
        cs.face.set_choices(
            [
                face_path.name
                for face_path in lib_path.get_files_paths(
                    self.faces_path, extensions=[".jpg", ".jpeg", ".png"]
                )
            ],
            none_choice_name="@misc.menu_select",
        )
    ...
    def on_cs_device(self, idx, device):
        ...
        if device is not None and state.device == device:
            ...
            cs.adjust_c.enable()
            cs.adjust_c.set_config(
                lib_csw.Number.Config(
                    min=1.0, max=2.0, step=0.01, decimals=2, allow_instant_update=True
                )
            )
            ...
            cs.adjust_x.enable()
            cs.adjust_x.set_config(
                lib_csw.Number.Config(
                    min=-0.5, max=0.5, step=0.01, decimals=2, allow_instant_update=True
                )
            )
            ...
            cs.animator_face_id.set_config(
                lib_csw.Number.Config(
                    min=0, max=16, step=1, decimals=0, allow_instant_update=True
                )
            )
            cs.animator_face_id.set_number(
                state.animator_face_id if state.animator_face_id is not None else 0
            )
    ...
    def on_cs_animator_face_id(self, animator_face_id):
        ...
        animator_face_id = state.animator_face_id = int(
            np.clip(animator_face_id, cfg.min, cfg.max)
        )
            ...
            if bcd is not None:
                ...
                if self.face_vector is None and self.target_face_img is not None:
                    rects = self.face_detector.extract(
                        self.target_face_img, threshold=0.5
                    )[0]
                    if len(rects) > 0:
                        ...
                        face_urect = FRect.sort_by_area_size(u_rects)[
                            0
                        ]  # sorted by largest

                        face_image, face_uni_mat = face_urect.cut(
                            self.target_face_img, 1.6, 192
                        )
                        lmrks = self.face_marker.extract(face_image)[0]
                        lmrks = lmrks[..., 0:2] / (192, 192)

                        face_ulmrks = FLandmarks2D.create(
                            ELandmarks2D.L106, lmrks
                        ).transform(face_uni_mat, invert=True)

                        face_align_img, _ = face_ulmrks.cut(
                            self.target_face_img,
                            state.adjust_c,
                            self.swap_model.get_face_vector_input_size(),
                            x_offset=state.adjust_x,
                            y_offset=state.adjust_y,
                        )
                        self.face_vector = self.swap_model.get_face_vector(
                            face_align_img
                        )
                    ...
                    for i, fsi in enumerate(bcd.get_face_swap_info_list()):
                        if state.animator_face_id == i:
                            ...
                            if face_align_image is not None:
                                ...
                                anim_image = swap_model.generate(
                                    face_align_image, self.face_vector
                                )
                                anim_image = (
                                    ImageProcessor(anim_image)
                                    .resize((W, H))
                                    .get_image("HWC")
                                )
                                ...
                                white_mask = np.full_like(
                                    anim_image, 255, dtype=np.uint8
                                )
