...
class FaceMergerWorker(BackendWorker):
    ...
    def on_start(
        ...
        cs.device.set_choices(
            ["CPU"] + lib_cl.get_available_devices_info(),
    ...
    def on_cs_device(self, idxs, device: lib_cl.DeviceInfo):
        ...
        if device is not None and state.device == device:
            ...
            cs.face_x_offset.set_config(
                lib_csw.Number.Config(
                    min=-0.5, max=0.5, step=0.001, decimals=3, allow_instant_update=True
                )
            )
            cs.face_x_offset.set_number(
                state.face_x_offset if state.face_x_offset is not None else 0.0
            )

            cs.face_y_offset.enable()
            cs.face_y_offset.set_config(
                lib_csw.Number.Config(
                    min=-0.5, max=0.5, step=0.001, decimals=3, allow_instant_update=True
                )
            )
            cs.face_y_offset.set_number(
                state.face_y_offset if state.face_y_offset is not None else 0.0
            )

            cs.face_scale.enable()
            cs.face_scale.set_config(
                lib_csw.Number.Config(
                    min=0.5, max=1.5, step=0.01, decimals=2, allow_instant_update=True
                )
            )
            cs.face_scale.set_number(
                state.face_scale if state.face_scale is not None else 1.0
            )

            cs.face_mask_source.enable()
            cs.face_mask_source.set_flag(
                state.face_mask_source if state.face_mask_source is not None else True
            )
            cs.face_mask_celeb.enable()
            cs.face_mask_celeb.set_flag(
                state.face_mask_celeb if state.face_mask_celeb is not None else True
            )
            cs.face_mask_lmrks.enable()
            cs.face_mask_lmrks.set_flag(
                state.face_mask_lmrks if state.face_mask_lmrks is not None else False
            )

            cs.face_mask_erode.enable()
            cs.face_mask_erode.set_config(
                lib_csw.Number.Config(
                    min=-400, max=400, step=1, decimals=0, allow_instant_update=True
                )
            )
            cs.face_mask_erode.set_number(
                state.face_mask_erode if state.face_mask_erode is not None else 5.0
            )

            cs.face_mask_blur.enable()
            cs.face_mask_blur.set_config(
                lib_csw.Number.Config(
                    min=0, max=400, step=1, decimals=0, allow_instant_update=True
                )
            )
            cs.face_mask_blur.set_number(
                state.face_mask_blur if state.face_mask_blur is not None else 25.0
            )
            ...
            cs.color_transfer.select(
                state.color_transfer if state.color_transfer is not None else "rct"
            )

            cs.interpolation.call_on_selected(self.on_cs_interpolation)
            cs.interpolation.enable()
            cs.interpolation.set_choices(
                ["bilinear", "bicubic", "lanczos4"], none_choice_name=None
            )
            cs.interpolation.select(
                state.interpolation if state.interpolation is not None else "bilinear"
            )

            cs.color_compression.enable()
            cs.color_compression.set_config(
                lib_csw.Number.Config(
                    min=0.0, max=127.0, step=0.1, decimals=1, allow_instant_update=True
                )
            )
            cs.color_compression.set_number(
                state.color_compression if state.color_compression is not None else 1.0
            )

            cs.face_opacity.enable()
            cs.face_opacity.set_config(
                lib_csw.Number.Config(
                    min=0.0, max=1.0, step=0.01, decimals=2, allow_instant_update=True
                )
            )
            cs.face_opacity.set_number(
                state.face_opacity if state.face_opacity is not None else 1.0
            )
        else:
            state.device = device
            self.save_state()
    ...
    def on_cs_face_x_offset(self, face_x_offset):
        ...
        face_x_offset = state.face_x_offset = float(
            np.clip(face_x_offset, cfg.min, cfg.max)
        )
    ...
    def on_cs_face_y_offset(self, face_y_offset):
        ...
        face_y_offset = state.face_y_offset = float(
            np.clip(face_y_offset, cfg.min, cfg.max)
        )
    ...
    def on_cs_mask_erode(self, face_mask_erode):
        ...
        face_mask_erode = state.face_mask_erode = int(
            np.clip(face_mask_erode, cfg.min, cfg.max)
        )
    ...
    def on_cs_mask_blur(self, face_mask_blur):
        ...
        face_mask_blur = state.face_mask_blur = int(
            np.clip(face_mask_blur, cfg.min, cfg.max)
        )
    ...
    def on_cs_color_compression(self, color_compression):
        ...
        color_compression = state.color_compression = float(
            np.clip(color_compression, cfg.min, cfg.max)
        )
    ...
    def on_cs_face_opacity(self, face_opacity):
        ...
        face_opacity = state.face_opacity = float(
            np.clip(face_opacity, cfg.min, cfg.max)
        )
    ...
    _cpu_interp = {
        "bilinear": ImageProcessor.Interpolation.LINEAR,
    ...
    def _merge_on_cpu(
        ...
        if state.face_mask_source:
            masks.append(
                ImageProcessor(face_align_mask_img).to_ufloat32().get_image("HW")
            )
        if state.face_mask_celeb:
            masks.append(
                ImageProcessor(face_swap_mask_img).to_ufloat32().get_image("HW")
            )
        if state.face_mask_lmrks:
            masks.append(
                ImageProcessor(face_align_lmrks_mask_img).to_ufloat32().get_image("HW")
            )

        masks_count = len(masks)
        if masks_count == 0:
            face_mask = np.ones(
                shape=(face_resolution, face_resolution), dtype=np.float32
            )
        else:
            face_mask = masks[0]
        ...
        # Combine face mask
        face_mask = (
            ImageProcessor(face_mask)
            .erode_blur(
                state.face_mask_erode, state.face_mask_blur, fade_to_border=True
            )
            .get_image("HWC")
        )
        frame_face_mask = (
            ImageProcessor(face_mask)
            .warp_affine(aligned_to_source_uni_mat, frame_width, frame_height)
            .clip2((1.0 / 255.0), 0.0, 1.0, 1.0)
            .get_image("HWC")
        )
        ...
        if state.color_transfer == "rct":
            face_swap_img = face_swap_ip.rct(
                like=face_align_img, mask=face_mask, like_mask=face_mask
            )

        frame_face_swap_img = face_swap_ip.warp_affine(
            aligned_to_source_uni_mat,
            frame_width,
            frame_height,
        ...
        if opacity == 1.0:
            out_merged_frame = ne.evaluate(
                "frame_image*(one_f-frame_face_mask) + frame_face_swap_img*frame_face_mask"
            )
        else:
            out_merged_frame = ne.evaluate(
    ...
    _gpu_interp = {
        "bilinear": lib_cl.EInterpolation.LINEAR,
        "bicubic": lib_cl.EInterpolation.CUBIC,
        "lanczos4": lib_cl.EInterpolation.LANCZOS4,
    }

    _n_mask_multiply_op_text = [
        f"float X = {'*'.join([f'(((float)I{i}) / 255.0)' for i in range(n)])}; O = (X <= 0.5 ? 0 : 1);"
        for n in range(5)
    ]
    ...
    def _merge_on_gpu(
        ...
        masks_count = len(masks)
        if masks_count == 0:
            face_mask_t = lib_cl.Tensor(
                shape=(face_resolution, face_resolution),
                dtype=np.float32,
                initializer=lib_cl.InitConst(1.0),
            )
        else:
            face_mask_t = lib_cl.any_wise(
                FaceMergerWorker._n_mask_multiply_op_text[masks_count],
                *masks,
                dtype=np.uint8,
            ).transpose((2, 0, 1))

        face_mask_t = lib_cl.binary_morph(
            face_mask_t,
            state.face_mask_erode,
            state.face_mask_blur,
            fade_to_border=True,
            dtype=np.float32,
        )
        face_swap_img_t = lib_cl.Tensor.from_value(face_swap_img).transpose(
            (2, 0, 1), op_text="O = ((O_TYPE)I) / 255.0", dtype=np.float32
        )

        if state.color_transfer == "rct":
            face_align_img_t = lib_cl.Tensor.from_value(face_align_img).transpose(
                (2, 0, 1), op_text="O = ((O_TYPE)I) / 255.0", dtype=np.float32
            )
            face_swap_img_t = lib_cl.rct(
                face_swap_img_t,
                face_align_img_t,
                target_mask_t=face_mask_t,
                source_mask_t=face_mask_t,
            )

        frame_face_mask_t = lib_cl.remap_np_affine(
            face_mask_t,
            aligned_to_source_uni_mat,
        ...
        frame_face_swap_img_t = lib_cl.remap_np_affine(
            face_swap_img_t,
            aligned_to_source_uni_mat,
            interpolation=interpolation,
            output_size=(frame_height, frame_width),
        ...
        frame_image_t = lib_cl.Tensor.from_value(frame_image).transpose(
            (2, 0, 1),
            op_text=(
                "O = ((float)I) / 255.0;" if frame_image.dtype == np.uint8 else None
            ),
        ...
        if opacity == 1.0:
            ...
            frame_final_t = lib_cl.any_wise(
                "O = I0*(1.0-I1) + I0*I1*(1.0-I3) + I2*I1*I3",
                frame_image_t,
                frame_face_mask_t,
                frame_face_swap_img_t,
                np.float32(opacity),
                ...
                if merged_frame is not None:
                    ...
                    for fsi_id, fsi in enumerate(fsi_list):
                        ...
                        if face_anim_img is not None:
                            ...
                            face_align_lmrks_mask_img = bcd.get_image(
                                fsi.face_align_lmrks_mask_name
                            )
                            face_align_mask_img = bcd.get_image(
                            ...
                            if all_is_not_None(
                                ...
                                aligned_to_source_uni_mat = (
                                    image_to_align_uni_mat.invert()
                                )
                                aligned_to_source_uni_mat = (
                                    aligned_to_source_uni_mat.source_translated(
                                        -state.face_x_offset, -state.face_y_offset
                                    )
                                )
                                aligned_to_source_uni_mat = aligned_to_source_uni_mat.source_scaled_around_center(
                                    state.face_scale, state.face_scale
                                )
                                aligned_to_source_uni_mat = (
                                    aligned_to_source_uni_mat.to_exact_mat(
                                        face_width,
                                        face_height,
                                        frame_width,
                                        frame_height,
                                ...
                                if state.device == "CPU":
                                    merged_frame = self._merge_on_cpu(
                                        merged_frame,
                                        face_resolution,
                                        face_align_img,
                                        face_align_mask_img,
                                        ...
                                        frame_width,
                                        frame_height,
                                        do_color_compression=do_color_compression,
                                    )
                                else:
                                    merged_frame = self._merge_on_gpu(
                                        merged_frame,
                                        face_resolution,
                                        face_align_img,
                                        face_align_mask_img,
