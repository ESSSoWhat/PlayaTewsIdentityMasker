...
class FileSourceWorker(BackendWorker):
    ...
    def on_cs_input_type_selected(self, idx, input_type):
        ...
        if input_type is not None:
            ...
            if input_type == InputType.IMAGE_SEQUENCE:
                ...
                cs.input_paths.set_config(
                    lib_csw.Paths.Config.ExistingFile(
                        caption="Video file",
                        suffixes=lib_player.VideoFilePlayer.SUPPORTED_VIDEO_FILE_SUFFIXES,
                    )
                )
    ...
    def on_cs_input_paths(self, paths, prev_paths):
        ...
        if input_path is not None:
            if (
                input_type == InputType.IMAGE_SEQUENCE
                or input_type == InputType.VIDEO_FILE
            ):
                cls_ = (
                    lib_player.ImageSequencePlayer
                    if input_type == InputType.IMAGE_SEQUENCE
                ...
                if err is None:
                    ...
                    if input_path == state.input_path and state.fp_state is not None:
                        ...
                        is_autorewind = fp.set_is_autorewind(
                            state.fp_state.is_autorewind
                        )
                    else:
                        state.input_path = input_path
                        state.fp_state = FPState()
                        target_width = state.fp_state.target_width = (
                            fp.get_target_width()
                        )
                        fps = state.fp_state.fps = fp.get_fps()
                        is_realtime = state.fp_state.is_realtime = fp.set_is_realtime(
                            True
                        )
                        is_autorewind = state.fp_state.is_autorewind = (
                            fp.set_is_autorewind(True)
                        )

                    cs.target_width.enable()
                    cs.target_width.set_config(
    ...
    def on_cs_target_width(self, target_width):
        state, cs = self.get_state(), self.get_control_sheet()
        target_width = state.fp_state.target_width = self.fp.set_target_width(
            target_width
        )
        cs.target_width.set_number(target_width)
        self.save_state()
    ...
    def on_cs_is_autorewind(self, is_autorewind):
        state, cs = self.get_state(), self.get_control_sheet()
        is_autorewind = state.fp_state.is_autorewind = self.fp.set_is_autorewind(
            is_autorewind
        )
        cs.is_autorewind.set_flag(is_autorewind)
        self.save_state()
