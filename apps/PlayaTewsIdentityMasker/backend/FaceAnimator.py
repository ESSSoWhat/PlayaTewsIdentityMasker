...
class FaceAnimatorWorker(BackendWorker):
    ...
    def on_start(
        ...
        cs.device.set_choices(
            LIA.get_available_devices(), none_choice_name="@misc.menu_select"
        )
        cs.device.select(state.device)

    def update_animatables(self):
        state, cs = self.get_state(), self.get_control_sheet()
        cs.animatable.set_choices(
            [
                animatable_path.name
                for animatable_path in lib_path.get_files_paths(
                    self.animatables_path, extensions=[".jpg", ".jpeg", ".png"]
                )
            ],
    ...
    def on_cs_device(self, idx, device):
        ...
        if device is not None and state.device == device:
            ...
            cs.relative_power.set_config(
                lib_csw.Number.Config(
                    min=0.0, max=2.0, step=0.01, decimals=2, allow_instant_update=True
                )
            )
            cs.relative_power.set_number(
                state.relative_power if state.relative_power is not None else 1.0
            )
    ...
    def on_cs_relative_power(self, relative_power):
        ...
        relative_power = state.relative_power = float(
            np.clip(relative_power, cfg.min, cfg.max)
        )
                        ...
                        if state.animator_face_id == i:
                            ...
                            if face_align_image is not None:
                                ...
                                if self.driving_ref_motion is None:
                                    self.driving_ref_motion = lia_model.extract_motion(
                                        face_align_image
                                    )

                                anim_image = lia_model.generate(
                                    self.animatable_img,
                                    face_align_image,
                                    self.driving_ref_motion,
