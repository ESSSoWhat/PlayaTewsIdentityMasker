...
_ResolutionType_names = {
    _ResolutionType.RES_320x240: "320x240",
...
_ResolutionType_wh = {
    _ResolutionType.RES_320x240: (320, 240),
...
_DriverType_names = {
    _DriverType.COMPATIBLE: "Compatible",
...
class CameraSourceWorker(BackendWorker):
    ...
    def on_start(self, weak_heap: BackendWeakHeap, bc_out: BackendConnection):
        ...
        cs.driver.enable()
        cs.driver.set_choices(
            _DriverType, _DriverType_names, none_choice_name="@misc.menu_select"
        )
        ...
        if platform.system() == "Windows":
            try:
                from xlib.api.win32 import dshow, ole32

                ole32.CoInitializeEx(0, 0)
                device_names = dshow.get_video_input_devices_names()
                print(f"🔍 Found {len(device_names)} DirectShow devices:")
                ...
                print(f"⚠️ Error getting DirectShow devices: {e}")
        ...
        cs.resolution.set_choices(
            _ResolutionType, _ResolutionType_names, none_choice_name=None
        )
        cs.resolution.select(
            state.resolution
        ...
        print(
            f"🔍 Camera source state: device_idx={state.device_idx}, driver={state.driver}"
        )

        if state.device_idx is not None and state.driver is not None:
            ...
            try:
                ...
                if vcap.isOpened():
                    print(f"✅ Camera {state.device_idx} opened successfully")
                    self.vcap = vcap
                    ...
                    actual_w = vcap.get(cv2.CAP_PROP_FRAME_WIDTH)
                    actual_h = vcap.get(cv2.CAP_PROP_FRAME_HEIGHT)
                    print(f"📊 Camera resolution: {actual_w}x{actual_h} (requested: {w}x{h})")

                else:
                    print(f"❌ Failed to open camera {state.device_idx}")
                    self.vcap = None
                    
            except Exception as e:
                print(f"❌ Error opening camera {state.device_idx}: {e}")
                self.vcap = None
            ...
            if self.vcap is not None and self.vcap.isOpened():
                ...
                cs.rotation.enable()
                cs.rotation.set_choices(
                    _RotationType, _RotationType_names, none_choice_name=None
                )
                cs.rotation.select(
                    state.rotation
                ...
                cs.flip_horizontal.set_flag(
                    state.flip_horizontal if state.flip_horizontal is not None else False
                )
                ...
                print(f"✅ Camera source initialized successfully")
            else:
                print(f"❌ Camera source initialization failed - no valid camera")
                # Disable controls if camera is not available
                cs.fps.disable()
                cs.rotation.disable()
                cs.flip_horizontal.disable()
                cs.open_settings.disable()
                cs.load_settings.disable()
                cs.save_settings.disable()
    ...
    def on_tick(self):
        if self.vcap is not None and not self.vcap.isOpened():
            print("⚠️ Camera connection lost, releasing...")
            self.set_vcap(None)
            ...
            try:
                ...
                if ret:
                    ...
                    if img is None or img.size == 0:
                        print("⚠️ Received empty frame from camera")
                        ret = False
                    else:
