...
class FaceMarkerWorker(BackendWorker):
    ...
    def on_start(
        ...
        cs.marker_type.select(
            state.marker_type
            if state.marker_type is not None
            else MarkerType.GOOGLE_FACEMESH
        )
        ...
        if state.marker_type == marker_type:
            ...
            if marker_type == MarkerType.OPENCV_LBF:
                ...
                cs.device.set_choices(
                    onnx_models.FaceMesh.get_available_devices(),
                ...
                cs.device.set_choices(
                    onnx_models.InsightFace2D106.get_available_devices(),
                    none_choice_name="@misc.menu_select",
                )
    ...
    def on_cs_devices(self, idx, device):
        ...
        if device is not None and (
            (
                marker_type == MarkerType.OPENCV_LBF
                and state.opencv_lbf_state.device == device
            )
            or (
                marker_type == MarkerType.GOOGLE_FACEMESH
                and state.google_facemesh_state.device == device
            )
            or (
                marker_type == MarkerType.INSIGHT_2D106
                and state.insightface_2d106_state.device == device
            )
        ):
            ...
            if state.marker_type == MarkerType.OPENCV_LBF:
                ...
                self.google_facemesh = onnx_models.FaceMesh(
                    state.google_facemesh_state.device
                )
            elif state.marker_type == MarkerType.INSIGHT_2D106:
                self.insightface_2d106 = onnx_models.InsightFace2D106(
                    state.insightface_2d106_state.device
                )

            cs.marker_coverage.enable()
            cs.marker_coverage.set_config(
                lib_csw.Number.Config(
                    min=0.1, max=3.0, step=0.1, decimals=1, allow_instant_update=True
                )
            )
            ...
            cs.temporal_smoothing.set_config(
                lib_csw.Number.Config(min=1, max=150, step=1, allow_instant_update=True)
            )
            cs.temporal_smoothing.set_number(
                marker_state.temporal_smoothing
                if marker_state.temporal_smoothing is not None
                else 1
            )
    ...
    def on_cs_marker_coverage(self, marker_coverage):
        ...
        marker_coverage = state.get_marker_state().marker_coverage = np.clip(
            marker_coverage, cfg.min, cfg.max
        )
        cs.marker_coverage.set_number(marker_coverage)
        self.save_state()
    ...
    def on_cs_temporal_smoothing(self, temporal_smoothing):
        ...
        temporal_smoothing = state.get_marker_state().temporal_smoothing = int(
    ...
    def on_tick(self):
        ...
        if self.pending_bcd is None:
            ...
            if bcd is not None:
                ...
                is_opencv_lbf = (
                    marker_type == MarkerType.OPENCV_LBF and self.opencv_lbf is not None
                )
                is_google_facemesh = (
                    marker_type == MarkerType.GOOGLE_FACEMESH
                    and self.google_facemesh is not None
                )
                is_insightface_2d106 = (
                    marker_type == MarkerType.INSIGHT_2D106
                    and self.insightface_2d106 is not None
                )
                is_marker_loaded = (
                    is_opencv_lbf or is_google_facemesh or is_insightface_2d106
                )
                ...
                if marker_type is not None:
                    ...
                    if frame_image is not None and is_marker_loaded:
                        ...
                        if marker_state.temporal_smoothing != 1 and len(
                            self.temporal_lmrks
                        ) != len(fsi_list):
                        ...
                        for face_id, fsi in enumerate(fsi_list):
                            if fsi.face_urect is not None:
                                ...
                                face_image, face_uni_mat = fsi.face_urect.cut(
                                    frame_image,
                                    marker_state.marker_coverage,
                                    (
                                        256
                                        if is_opencv_lbf
                                        else (
                                ...
                                if is_opencv_lbf:
                                    ...
                                    lmrks = self.insightface_2d106.extract(face_image)[
                                        0
                                    ]
                                ...
                                if marker_state.temporal_smoothing != 1:
                                    if (
                                        ...
                                        or len(self.temporal_lmrks[face_id]) == 0
                                    ):
                                        self.temporal_lmrks[face_id].append(lmrks)
                                    self.temporal_lmrks[face_id] = self.temporal_lmrks[
                                        face_id
                                    ][-marker_state.temporal_smoothing :]
                                ...
                                face_ulmrks = FLandmarks2D.create(
                                    (
                                        ELandmarks2D.L68
                                        if is_opencv_lbf
                                        else (
                                ...
                                face_ulmrks = face_ulmrks.transform(
                                    face_uni_mat, invert=True
                                )
