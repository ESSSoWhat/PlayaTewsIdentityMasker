...
class FaceAlignerWorker(BackendWorker):
    ...
    def on_start(
        ...
        cs.align_mode.select(
            state.align_mode if state.align_mode is not None else AlignMode.FROM_POINTS
        )

        cs.face_coverage.enable()
        cs.face_coverage.set_config(
            lib_csw.Number.Config(
                min=0.1, max=8.0, step=0.1, decimals=1, allow_instant_update=True
            )
        )
        ...
        cs.resolution.set_config(
            lib_csw.Number.Config(
                min=16, max=1024, step=16, decimals=0, allow_instant_update=True
            )
        )
        ...
        cs.exclude_moving_parts.set_flag(
            state.exclude_moving_parts
            if state.exclude_moving_parts is not None
            else True
        )
        ...
        cs.freeze_z_rotation.set_flag(
            state.freeze_z_rotation if state.freeze_z_rotation is not None else False
        )

        cs.x_offset.enable()
        cs.x_offset.set_config(
            lib_csw.Number.Config(
                min=-10, max=10, step=0.01, decimals=2, allow_instant_update=True
            )
        )
    ...
    def on_cs_resolution(self, resolution):
        ...
        resolution = state.resolution = int(
            np.clip((resolution // 16) * 16, cfg.min, cfg.max)
        )
                    ...
                    for face_id, fsi in enumerate(bcd.get_face_swap_info_list()):
                        ...
                        if face_ulmrks is not None:
                            ...
                            if state.align_mode == AlignMode.FROM_RECT:
                                face_align_img, uni_mat = fsi.face_urect.cut(
                                    frame_image,
                                ...
                                face_align_img, uni_mat = face_ulmrks.cut(
                                    frame_image,
                                    state.face_coverage
                                    + (1.0 if state.head_mode else 0.0),
                                    state.resolution,
                                    ...
                                    y_offset=state.y_offset
                                    - 0.08
                                    + (-0.50 if state.head_mode else 0.0),
                            ...
                            fsi.face_align_image_name = (
                                f"{frame_image_name}_{face_id}_aligned"
                            )
                            ...
                            face_align_lmrks_mask_img = (
                                fsi.face_align_ulmrks.get_convexhull_mask(
