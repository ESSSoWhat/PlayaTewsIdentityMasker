...
class FaceDetector(BackendHost):
    ...
    def get_weak_heap(self) -> BackendWeakHeap:
        return self._weak_heap

    def get_bc_out(self) -> BackendConnection:
        return self._bc_out
...
class FaceDetectorWorker(BackendWorker):
    ...
    def on_start(
        ...
        cs.detector_type.set_choices(
            DetectorType, DetectorTypeNames, none_choice_name=None
        )
        cs.detector_type.select(
            state.detector_type
            if state.detector_type is not None
            else DetectorType.YOLOV5
        )
        ...
        if state.detector_type == detector_type:
            if detector_type == DetectorType.CENTER_FACE:
                ...
                cs.device.set_choices(
                    onnx_models.CenterFace.get_available_devices(),
                ...
                cs.device.enable()
                cs.device.set_choices(
                    onnx_models.S3FD.get_available_devices(),
                    none_choice_name="@misc.menu_select",
                )
                ...
                cs.device.enable()
                cs.device.set_choices(
                    onnx_models.YoloV5Face.get_available_devices(),
                    none_choice_name="@misc.menu_select",
                )
    ...
    def on_cs_devices(self, idx, device):
        ...
        if (
            device is not None
            and (
                detector_type == DetectorType.CENTER_FACE
                and state.center_face_state.device == device
            )
            or (
                detector_type == DetectorType.S3FD and state.S3FD_state.device == device
            )
            or (
                detector_type == DetectorType.YOLOV5
                and state.YoloV5_state.device == device
            )
        ):
            detector_state = state.get_detector_state()

            cs.max_faces.enable()
            cs.max_faces.set_config(
                lib_csw.Number.Config(
                    min=0, max=256, step=1, decimals=0, allow_instant_update=True
                )
            )
            cs.max_faces.set_number(detector_state.max_faces or 1)

            if detector_type in [
                DetectorType.CENTER_FACE,
                DetectorType.S3FD,
                DetectorType.YOLOV5,
            ]:
                cs.fixed_window_size.enable()
                cs.fixed_window_size.set_config(
                    lib_csw.Number.Config(
                        min=0,
                        max=4096,
                        step=32,
                ...
                cs.fixed_window_size.set_number(
                    detector_state.fixed_window_size
                    if detector_state.fixed_window_size is not None
                    else 480
                )

                cs.threshold.enable()
                cs.threshold.set_config(
                    lib_csw.Number.Config(
                        min=0.01,
                        max=1.0,
                        step=0.01,
                ...
                cs.threshold.set_number(
                    detector_state.threshold
                    if detector_state.threshold is not None
                    else 0.5
                )
                ...
                cs.sort_by.select(
                    detector_state.sort_by
                    if detector_state.sort_by is not None
                    else FaceSortBy.LARGEST
                )

                cs.temporal_smoothing.enable()
                cs.temporal_smoothing.set_config(
                    lib_csw.Number.Config(
                        min=1, max=150, step=1, allow_instant_update=True
                    )
                )
                cs.temporal_smoothing.set_number(
                    detector_state.temporal_smoothing
                    if detector_state.temporal_smoothing is not None
                    else 1
                )
    ...
    def on_cs_fixed_window_size(self, fixed_window_size):
        ...
        fixed_window_size = state.get_detector_state().fixed_window_size = int(
            np.clip(fixed_window_size, cfg.min, cfg.max)
        )
    ...
    def on_cs_threshold(self, threshold):
        ...
        threshold = state.get_detector_state().threshold = np.clip(
    ...
    def on_cs_max_faces(self, max_faces):
        ...
        max_faces = state.get_detector_state().max_faces = np.clip(
            max_faces, cfg.min, cfg.max
        )
    ...
    def on_cs_temporal_smoothing(self, temporal_smoothing):
        ...
        temporal_smoothing = state.get_detector_state().temporal_smoothing = int(
            np.clip(temporal_smoothing, cfg.min, cfg.max)
        )
        ...
        if self.pending_bcd is None:
            ...
            if bcd is not None:
                ...
                if (
                    ...
                    or (detector_type == DetectorType.S3FD and self.S3FD is not None)
                    or (
                        detector_type == DetectorType.YOLOV5
                        and self.YoloV5Face is not None
                    )
                ):
                    detector_state = state.get_detector_state()
                    ...
                    if frame_image is not None:
                        ...
                        rects = []
                        if detector_type == DetectorType.CENTER_FACE:
                            rects = self.CenterFace.extract(
                                frame_image,
                                threshold=detector_state.threshold,
                                fixed_window=detector_state.fixed_window_size,
                            )[0]
                        elif detector_type == DetectorType.S3FD:
                            rects = self.S3FD.extract(
                                frame_image,
                                threshold=detector_state.threshold,
                                fixed_window=detector_state.fixed_window_size,
                            )[0]
                        elif detector_type == DetectorType.YOLOV5:
                            rects = self.YoloV5Face.extract(
                                frame_image,
                                threshold=detector_state.threshold,
                                fixed_window=detector_state.fixed_window_size,
                            )[0]
                        ...
                        rects = [
                            FRect.from_ltrb((l / W, t / H, r / W, b / H))
                        ...
                        if len(rects) != 0:
                            ...
                            for face_id, face_urect in enumerate(rects):
                                if detector_state.temporal_smoothing != 1:
                                    if (
                                        not is_frame_reemitted
                                        or len(self.temporal_rects[face_id]) == 0
                                    ):
                                        self.temporal_rects[face_id].append(
                                            face_urect.as_4pts()
                                        )

                                    self.temporal_rects[face_id] = self.temporal_rects[
                                        face_id
                                    ][-detector_state.temporal_smoothing :]

                                    face_urect = FRect.from_4pts(
                                        np.mean(self.temporal_rects[face_id], 0)
                                    )
