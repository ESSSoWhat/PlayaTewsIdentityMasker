...
class MemoryOptimizedFaceSwapWorker(BackendWorker):
    ...
    def on_start(self, weak_heap : BackendWeakHeap, reemit_frame_signal : BackendSignal, bc_in : BackendConnection, bc_out : BackendConnection, dfm_models_path : Path):
        ...
        cs.enable_preprocessing_cache.call_on_flag(self.on_cs_enable_preprocessing_cache)
        cs.enable_postprocessing_cache.call_on_flag(self.on_cs_enable_postprocessing_cache)
    ...
    def _memory_monitor_loop(self):
        ...
        while self.memory_monitoring:
            try:
                ...
                elif memory.percent < 60 and memory.available > 4 * 1024**3:  # 4GB available
                    # Low memory usage, increase cache
                    new_size = min(4096 * 1024 * 1024, self.ram_cache.max_size_bytes * 2)  # Max 4GB
                    self.ram_cache.max_size_bytes = new_size
    ...
    def _get_cache_key(self, face_align_image: np.ndarray, model_state) -> str:
        ...
        params_hash = hashlib.md5(str(sorted(model_state.__dict__.items())).encode()).hexdigest()
        return f"{image_hash}_{params_hash}"

    def _preprocess_face_align_image(self, face_align_image: np.ndarray, model_state) -> np.ndarray:
        """Preprocess face align image with caching"""
        if not hasattr(self.get_state(), 'enable_preprocessing_cache') or not self.get_state().enable_preprocessing_cache:
    ...
    def _apply_preprocessing(self, face_align_image: np.ndarray, model_state) -> np.ndarray:
    ...
    def _postprocess_face_swap_result(self, celeb_face: np.ndarray, model_state) -> np.ndarray:
        """Postprocess face swap result with caching"""
        if not hasattr(self.get_state(), 'enable_postprocessing_cache') or not self.get_state().enable_postprocessing_cache:
    ...
    def _apply_postprocessing(self, celeb_face: np.ndarray, model_state) -> np.ndarray:
        ...
        if post_gamma_red != 1.0 or post_gamma_blue != 1.0 or post_gamma_green != 1.0:
            return ImageProcessor(celeb_face).gamma(post_gamma_red, post_gamma_blue, post_gamma_green).get_image('HWC')
        
        return celeb_face

    def _process_face_swap_with_ram_cache(self, face_align_image: np.ndarray, model_state, dfm_model) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        optimized_image = self._preprocess_face_align_image(face_align_image, model_state)
    ...
    def on_tick(self):
        ...
        if self.dfm_model_initializer is not None:
            ...
            if events.new_status_downloading:
                ...
                cs.ram_cache_size.set_config(lib_csw.Number.Config(min=512, max=8192, step=256, decimals=0, allow_instant_update=True))
                cs.ram_cache_size.set_number(state.ram_cache_size if state.ram_cache_size is not None else 2048)

                cs.enable_preprocessing_cache.enable()
                cs.enable_preprocessing_cache.set_flag(state.enable_preprocessing_cache if state.enable_preprocessing_cache is not None else True)

                cs.enable_postprocessing_cache.enable()
                cs.enable_postprocessing_cache.set_flag(state.enable_postprocessing_cache if state.enable_postprocessing_cache is not None else True)

                cs.parallel_processing.enable()
                cs.parallel_processing.set_flag(state.parallel_processing if state.parallel_processing is not None else True)
                ...
                if all_is_not_None(dfm_model, model_state):
                    for i, fsi in enumerate(bcd.get_face_swap_info_list()):
                        ...
                        if face_align_image is not None:
                            # Process face swap with RAM caching
                            celeb_face, celeb_face_mask_img, face_align_mask_img = self._process_face_swap_with_ram_cache(
