...
class QFaceDetector(QBackendPanel):
    def __init__(self, backend: FaceDetector):
        ...
        q_detected_faces = self._q_detected_faces = qtx.QXCollapsibleSection(
            L("@QFaceDetector.detected_faces"),
            qtx.QXVBoxLayout([face_coords_label]),
            is_opened=True,
        )
        ...
        q_fixed_window_size = QSpinBoxCSWNumber(
            cs.fixed_window_size,
            reflect_state_widgets=[q_fixed_window_size_label, q_detected_faces],
        )
        ...
        row += 1
        grid_l.addWidget(
            q_threshold_label, row, 0, 1, 2, alignment=qtx.AlignRight | qtx.AlignVCenter
        )
        grid_l.addWidget(q_threshold, row, 2, 1, 2, alignment=qtx.AlignLeft)
        row += 1
        grid_l.addLayout(
            qtx.QXHBoxLayout([q_max_faces_label, 5, q_max_faces]),
            row,
            0,
            1,
            2,
        ...
        grid_l.addLayout(
            qtx.QXHBoxLayout([q_sort_by_label, 5, q_sort_by]),
            row,
            2,
            1,
            2,
        ...
        row += 1
        grid_l.addLayout(
            qtx.QXHBoxLayout([q_temporal_smoothing_label, 5, q_temporal_smoothing]),
            row,
            0,
            1,
            4,
        ...
        row += 1
        super().__init__(
            backend, L("@QFaceDetector.module_title"), layout=qtx.QXVBoxLayout([grid_l])
        )

    def _on_backend_state_change(
        self, backend, started, starting, stopping, stopped, busy
    ):
        super()._on_backend_state_change(
            backend, started, starting, stopping, stopped, busy
        )
    ...
    def _on_timer_10ms(self):
        try:
            if (
                self._q_detected_faces.is_opened()
                and not self.get_top_QXWindow().is_minimized()
            ):
                    ...
                    if bcd is not None:
                        ...
                        for face_id, fsi in enumerate(bcd.get_face_swap_info_list()):
                            ...
                            if fsi.face_urect is not None:
                                l, t, r, b = fsi.face_urect.as_ltrb_bbox(
                                    frame_image_w_h
                                ).astype(np.int)
                                info_str += f"[{l},{t},{r},{b}]"
