...
class QBCFaceAlignViewer(qtx.QXCollapsibleSection):
    def __init__(
        ...
        super().__init__(
            title=L("@QBCFaceAlignViewer.title"),
                ...
                for fsi in bcd.get_face_swap_info_list():
                    ...
                    if face_image is not None:
                        ...
                        if fsi.face_align_ulmrks is not None:
                            lmrks_layer = np.zeros(
                                (self._preview_width, self._preview_width, 4),
                                dtype=np.uint8,
                            )
                            ...
                            if (
                                fsi.face_urect is not None
                                and fsi.image_to_align_uni_mat is not None
                            ):
                                aligned_uni_rect = fsi.face_urect.transform(
                                    fsi.image_to_align_uni_mat
                                )
