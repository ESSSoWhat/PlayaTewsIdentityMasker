...
class QErrorCSWError(QCSWControl):
    ...
    def __init__(self, csw_error: lib_csw.Error.Client):
        ...
        self._label_warning = qtx.QXLabel(
            image=QXImageDB.warning_outline("red"),
            scaled_contents=True,
            size_policy=("fixed", "fixed"),
            fixed_size=(32, 32),
        )

        self._label = qtx.QXTextEdit(
            font=QXFontDB.get_default_font(size=7), read_only=True, fixed_height=80
        )

        super().__init__(
            csw_control=csw_error,
