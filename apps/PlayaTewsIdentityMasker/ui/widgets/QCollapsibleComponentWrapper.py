...
class QCollapsibleComponentWrapper(QXCollapsibleSection):
    ...
    def _count_component_settings(self, component):
        ...
        if hasattr(component, 'findChildren'):
            try:
                from PyQt5.QtWidgets import QSpinBox, QComboBox, QCheckBox, QSlider, QLineEdit
    ...
    def set_auto_collapse(self, enabled=True):
        ...
        if enabled and self._count_component_settings(self.component) <= self.auto_collapse_threshold:
            self.close()
        elif not enabled:
            self.open()
...
def group_small_components(title, components, max_visible=3):
    ...
    return QSmartCollapsibleGroup(
        ...
        max_visible_components=max_visible
    )


def create_optimized_face_processing_group(face_marker, face_animator, face_swap_insight, face_swap_dfm):
