...
class QBackendPanel(qtx.QXWidget):
    ...
    def __init__(
        self, backend: BackendHost, name: str, layout, content_align_top=False
    ):
        super().__init__()
        ...
        btn_on_off = self._btn_on_off = qtx.QXPushButton(
            tooltip_text=L("@QBackendPanel.start"),
            released=self._on_btn_on_off_released,
            fixed_width=20,
        )

        btn_reset_state = self._btn_reset_state = qtx.QXPushButton(
            image=QXImageDB.settings_reset_outline("gray"),
            released=self._on_btn_reset_state_released,
            tooltip_text=L("@QBackendPanel.reset_settings"),
            fixed_width=20,
        )

        fps_label = self._fps_label = qtx.QXLabel()

        bar_widget = self._bar_widget = qtx.QXFrameHBox(
            widgets=[
                btn_on_off,
                1,
                btn_reset_state,
        ...
        content_widget = self._content_widget = qtx.QXFrameHBox(
            [layout], contents_margins=2, enabled=False
        )
    ...
    def _on_backend_state_change(
        ...
        if busy or starting or stopping:
            btn_on_off.set_image_sequence(
                QXImageSequenceDB.icon_loading("yellow"), loop_count=0
            )
        elif started:
