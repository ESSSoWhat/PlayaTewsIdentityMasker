...
from .widgets.QButtonCSWDynamicSingleSwitch import QButtonCSWDynamicSingleSwitch
...
class QFileSource(QBackendPanel):
    def __init__(self, backend: FileSource):
        ...
        q_input_type = QButtonCSWDynamicSingleSwitch(
            cs.input_type, horizontal=True, radio_buttons=True
        )
        ...
        q_fps_label = QLabelPopupInfo(
            label=L("@QFileSource.fps"), popup_info_text=L("@QFileSource.help.fps")
        )
        ...
        row += 1
        grid_l.addLayout(
            qtx.QXHBoxLayout([q_is_realtime_label, 5, q_is_realtime, 5, q_fps_label]),
            row,
            0,
            1,
            2,
