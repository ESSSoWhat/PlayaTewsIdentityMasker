...
class QFaceAnimator(QBackendPanel):
    def __init__(self, backend: FaceAnimator, animatables_path: Path):
        ...
        q_animatable = QComboBoxCSWDynamicSingleSwitch(
            cs.animatable, reflect_state_widgets=[q_animatable_label, btn_open_folder]
        )
        ...
        q_relative_power_label = QLabelPopupInfo(
            label=L("@QFaceAnimator.relative_power")
        )
        q_relative_power = QSliderCSWNumber(
            cs.relative_power, reflect_state_widgets=[q_relative_power_label]
        )
        ...
        q_reset_reference_pose = QXPushButtonCSWSignal(
            cs.reset_reference_pose, text=L("@QFaceAnimator.reset_reference_pose")
        )
        ...
        row += 1
        grid_l.addWidget(
            q_animatable_label, row, 0, alignment=qtx.AlignRight | qtx.AlignVCenter
        )
        grid_l.addLayout(
            qtx.QXHBoxLayout(
        ...
        row += 1
        grid_l.addWidget(
            q_relative_power_label, row, 0, alignment=qtx.AlignRight | qtx.AlignVCenter
        )
        ...
        super().__init__(
            backend, L("@QFaceAnimator.module_title"), layout=qtx.QXVBoxLayout([grid_l])
        )

    def _btn_open_folder_released(self):
        qtx.QDesktopServices.openUrl(
            qtx.QUrl.fromLocalFile(str(self._animatables_path))
        )
