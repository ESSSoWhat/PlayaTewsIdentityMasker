...
class QBackupManagerUI(qtx.QXWidget):
    ...
    def _setup_ui(self):
        ...
        # Title
        title_label = qtx.QXLabel(
            text="UI Layout Backup Manager",
            font=qtx.QXFontDB.get_default_font(size=14, bold=True),
        )
    ...
    def _create_backup_section(self) -> qtx.QXWidget:
        ...
        self.backup_name_edit = qtx.QXLineEdit(
            placeholder_text="Enter backup name (optional)"
        )
        ...
        self.backup_desc_edit = qtx.QXLineEdit(
        ...
        # Create button
        self.create_backup_btn = qtx.QXPushButton(
            text="Create Backup", on_clicked=self._on_create_backup
        )
    ...
    def _create_backup_list_section(self) -> qtx.QXWidget:
        ...
        # Backup list
        self.backup_list = qtx.QXListWidget(
            on_item_selection_changed=self._on_backup_selected
        )
        layout.addWidget(self.backup_list)

        # Refresh button
        refresh_btn = qtx.QXPushButton(
            text="Refresh List", on_clicked=self._refresh_backup_list
        )
    ...
    def _create_actions_section(self) -> qtx.QXWidget:
        ...
        # Restore button
        self.restore_btn = qtx.QXPushButton(
            text="Restore Selected", on_clicked=self._on_restore_backup
        )
        self.restore_btn.setEnabled(False)
        layout.addWidget(self.restore_btn)

        # Delete button
        self.delete_btn = qtx.QXPushButton(
            text="Delete Selected", on_clicked=self._on_delete_backup
        )
        self.delete_btn.setEnabled(False)
        layout.addWidget(self.delete_btn)

        # Export button
        self.export_btn = qtx.QXPushButton(
            text="Export Selected", on_clicked=self._on_export_backup
        )
    ...
    def _on_backup_selected(
        self, current_item: qtx.QXListWidgetItem, previous_item: qtx.QXListWidgetItem
    ):
    ...
    def _on_restore_backup(self):
        ...
        try:
            ...
            reply = qtx.QXMessageBox.question(
                ...
                qtx.QXMessageBox.StandardButton.Yes
                | qtx.QXMessageBox.StandardButton.No,
            )
