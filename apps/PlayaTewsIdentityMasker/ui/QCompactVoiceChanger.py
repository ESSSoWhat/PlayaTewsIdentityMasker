...
class QCompactVoiceChanger(QWidget):
    ...
    def __init__(self, cs_voice_changer):
        ...
        for preset_name, effect_name in presets:
            ...
            btn.clicked.connect(
                lambda checked, name=effect_name: self._apply_preset(name)
            )
        ...
        try:
            self.q_pitch_shift = QDoubleSpinBoxCSWNumber(
                self.cs_voice_changer.pitch_shift
            )
        ...
        if effect_name in effect_mapping:
            ...
            if hasattr(self.cs_voice_changer, param_name):
                try:
                    ...
                    if effect_name == "Deep Voice" and hasattr(
                        self.cs_voice_changer, "pitch_shift"
                    ):
                        self.cs_voice_changer.pitch_shift.set_number(-3.0)
                    elif effect_name == "Pitch Up" and hasattr(
                        self.cs_voice_changer, "pitch_shift"
                    ):
                        self.cs_voice_changer.pitch_shift.set_number(3.0)
                    elif effect_name == "Robot" and hasattr(
                        self.cs_voice_changer, "robot_rate"
                    ):
                        self.cs_voice_changer.robot_rate.set_number(0.1)
                    elif effect_name == "Echo" and hasattr(
                        self.cs_voice_changer, "echo_delay"
                    ):
                    ...
                    print(
                        f"Warning: Could not apply voice effect via control sheet: {e}"
                    )
            ...
            try:
                if param_name == "effect_type" and hasattr(self, "q_effect_type"):
                    if isinstance(self.q_effect_type, QComboBox):
                        ...
                        if index >= 0:
                            ...
                            if effect_name in alt_names:
                                index = self.q_effect_type.findText(
                                    alt_names[effect_name]
                                )
                                if index >= 0:
                                    self.q_effect_type.setCurrentIndex(index)
                    ...
                    print(
                        f"🎤 Voice effect not applied: {effect_name} (parameter {param_name} not found)"
                    )
            except Exception as e:
