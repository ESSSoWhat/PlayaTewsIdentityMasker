...
class QStreamOutput(QBackendPanel):
    def __init__(self, backend: StreamOutput):
        ...
        q_aligned_face_id = QSpinBoxCSWNumber(
            cs.aligned_face_id, reflect_state_widgets=[q_aligned_face_id_label]
        )
        ...
        q_save_sequence_path = QPathEditCSWPaths(
            cs.save_sequence_path,
            reflect_state_widgets=[q_target_delay_label, q_save_sequence_path_label],
        )
        ...
        grid_l.addWidget(
            q_average_fps, row, 1, 1, 2, alignment=qtx.AlignLeft | qtx.AlignVCenter
        )
        row += 1
        grid_l.addWidget(
            q_source_type_label,
        ...
        grid_l.addWidget(
            q_source_type, row, 1, 1, 1, alignment=qtx.AlignLeft | qtx.AlignVCenter
        )
        grid_l.addWidget(q_show_hide_window, row, 2, 1, 1)

        row += 1
        grid_l.addWidget(
            q_aligned_face_id_label,
            row,
            0,
            1,
            1,
        ...
        grid_l.addWidget(
            q_aligned_face_id, row, 1, 1, 2, alignment=qtx.AlignLeft | qtx.AlignVCenter
        )
        row += 1
        grid_l.addWidget(
        ...
        grid_l.addWidget(
            q_target_delay, row, 1, 1, 2, alignment=qtx.AlignLeft | qtx.AlignVCenter
        )
        row += 1
        ...
        row += 1
        grid_l.addLayout(
            qtx.QXHBoxLayout([q_save_fill_frame_gap, 4, q_save_fill_frame_gap_label]),
            row,
            1,
            1,
            2,
        ...
        row += 1
        grid_l.addLayout(
            qtx.QXHBoxLayout([q_is_streaming, 4, q_is_streaming_label]),
            row,
            0,
            1,
            1,
        ...
        grid_l.addLayout(
            qtx.QXHBoxLayout([q_stream_addr, qtx.QXLabel(text=":"), q_stream_port]),
            row,
            1,
            1,
            2,
        ...
        super().__init__(backend, L("@QStreamOutput.module_title"), layout=grid_l)
