...
class QEnhancedLiveSwap(qtx.QXWidget):
    ...
    def _setup_backend_components(self):
        ...
        backend_db          = self.backend_db          = backend.BackendDB( self.settings_dirpath / 'states.dat' )
        ...
        try:
            ...
            stream_output  = self.stream_output  = EnhancedStreamOutput (weak_heap=backend_weak_heap, reemit_frame_signal=reemit_frame_signal, bc_in=face_merger_bc_out, save_default_path=self.userdata_path, backend_db=backend_db)
        except ImportError:
            stream_output  = self.stream_output  = backend.StreamOutput (weak_heap=backend_weak_heap, reemit_frame_signal=reemit_frame_signal, bc_in=face_merger_bc_out, save_default_path=self.userdata_path, backend_db=backend_db)
    ...
    def _setup_ui_components(self):
        ...
        self.q_face_animator  = QFaceAnimator(self.face_animator, animatables_path=self.userdata_path / 'animatables')
        self.q_face_swap_insight = QFaceSwapInsight(self.face_swap_insight, faces_path=self.userdata_path / 'animatables')
        self.q_face_swap_dfm  = QFaceSwapDFM(self.face_swap_dfm, dfm_models_path=self.userdata_path / 'dfm_models')
        ...
        # Create viewers
        self.q_ds_frame_viewer = QBCFrameViewer(self.backend_weak_heap, self.backend_connections['multi_sources'])
        self.q_ds_fa_viewer    = QBCFaceAlignViewer(self.backend_weak_heap, self.backend_connections['face_aligner'], preview_width=256)
        self.q_ds_fc_viewer    = QBCFaceSwapViewer(self.backend_weak_heap, self.backend_connections['face_merger'], preview_width=256)
        self.q_ds_merged_frame_viewer = QBCMergedFrameViewer(self.backend_weak_heap, self.backend_connections['face_merger'])
    ...
    def _setup_enhanced_connections(self):
        ...
        if hasattr(self.q_ds_merged_frame_viewer, 'frame_ready'):
            self.q_ds_merged_frame_viewer.frame_ready.connect(self.enhanced_video_display.update_video_frame)
        ...
        if hasattr(self.modern_control_panel, 'face_swap_toggled'):
            self.modern_control_panel.face_swap_toggled.connect(self._on_face_swap_toggle)
    ...
    def _setup_performance_monitoring(self):
        """Setup performance monitoring"""
        import time
        self.performance_timer = qtx.QXTimer(interval_ms=1000, timeout=self._update_performance_metrics)
    ...
    def _update_performance_metrics(self):
        ...
        try:
            ...
            if hasattr(self, 'status_label'):
                self.status_label.setText(f"FPS: {fps} | Memory: {memory_gb:.1f}GB | CPU: {cpu_percent:.0f}%")
...
class QEnhancedDFLAppWindow(qtx.QXWindow):
    ...
    def __init__(self, userdata_path, settings_dirpath):
        ...
        self.timer_5ms = qtx.QXTimer(interval_ms=5, timeout=self.q_enhanced_live_swap._on_timer_5ms)
...
class PlayaTewsIdentityMaskerEnhancedApp(qtx.QXMainApplication):
    ...
    def on_reinitialize(self):
        ...
        self.main_window = QEnhancedDFLAppWindow(self.userdata_path, self.settings_dirpath)
