...
from .streaming import (
    ...
    get_platform_config, validate_stream_config
)
...
class QOBSSceneManager(qtx.QXWidget):
    ...
    def _init_ui(self):
        ...
        self.remove_scene_btn = qtx.QXPushButton(text="Remove Scene", clicked=self._remove_scene)
        self.duplicate_scene_btn = qtx.QXPushButton(text="Duplicate Scene", clicked=self._duplicate_scene)
        ...
        self.source_down_btn = qtx.QXPushButton(text="↓", clicked=self._move_source_down)
    ...
    def _duplicate_scene(self):
        ...
        if current_row >= 0:
            ...
            new_scene = Scene(
                name=f"{current_scene.name} Copy",
                sources=current_scene.sources.copy()
            )
            self.scenes.append(new_scene)
    ...
    def _move_source_up(self):
        ...
        if current_row > 0 and 0 <= self.current_scene_index < len(self.scenes):
            ...
            sources[current_row], sources[current_row - 1] = sources[current_row - 1], sources[current_row]
            self._update_source_list()
            self.source_list.setCurrentRow(current_row - 1)
            
    def _move_source_down(self):
        current_row = self.source_list.currentRow()
        if (current_row >= 0 and current_row < self.source_list.count() - 1 and 
            0 <= self.current_scene_index < len(self.scenes)):
            sources = self.scenes[self.current_scene_index].sources
            sources[current_row], sources[current_row + 1] = sources[current_row + 1], sources[current_row]
            self._update_source_list()
            self.source_list.setCurrentRow(current_row + 1)
...
class QOBSStreamManager(qtx.QXWidget):
    ...
    def _init_ui(self):
        ...
        self.start_streaming_btn = qtx.QXPushButton(
            ...
            clicked=self._toggle_streaming
        )
        ...
        self.start_recording_btn = qtx.QXPushButton(
            ...
            clicked=self._toggle_recording
        )
        ...
        platforms_label.setStyleSheet("font-weight: bold; font-size: 12px; margin-top: 10px;")
        layout.addWidget(platforms_label)
        
        # Platform controls
        platform_controls_layout = qtx.QXHBoxLayout()
        
        self.add_platform_btn = qtx.QXPushButton(text="Add Platform", clicked=self._add_platform)
        self.remove_platform_btn = qtx.QXPushButton(text="Remove Platform", clicked=self._remove_platform)
        self.configure_platform_btn = qtx.QXPushButton(text="Configure", clicked=self._configure_platform)
        ...
        recording_label.setStyleSheet("font-weight: bold; font-size: 12px; margin-top: 10px;")
    ...
    def _start_streaming(self):
        ...
        for config in self.stream_configs:
            ...
            if errors:
                qtx.QXMessageBox.warning(self, "Configuration Error", 
        ...
        if success:
            ...
            self.status_label.setStyleSheet("color: #f44336; font-weight: bold; font-size: 11px; margin: 4px;")
        else:
...
class QOBSMainPreview(qtx.QXWidget):
    ...
    def _init_ui(self):
        ...
        self.preview_area.setStyleSheet(self.preview_area.styleSheet() + "color: #666; font-size: 16px;")
...
class QOBSStyleLiveSwap(qtx.QXWidget):
    ...
    def _init_ui(self):
        ...
        face_controls_label.setStyleSheet("font-weight: bold; font-size: 12px; margin-bottom: 5px;")
...
class OBSStyleDeepFaceLiveApp(qtx.QXMainApplication):
    def __init__(self, userdata_path):
        ...
        super().__init__(app_name='OBS-Style DeepFaceLive', settings_dirpath=settings_dirpath)
    ...
    def initialize(self):
        ...
        if self._obs_wnd is None:
            self._obs_wnd = QOBSStyleAppWindow(
                ...
                settings_dirpath=self.settings_dirpath
            )
