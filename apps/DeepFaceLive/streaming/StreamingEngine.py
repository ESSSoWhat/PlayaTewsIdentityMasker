...
class StreamingEngine:
    ...
    def remove_stream_config(self, platform: StreamPlatform):
        ...
        self.stream_configs = [
            config for config in self.stream_configs 
...
class RecordingEngine:
    ...
    def start_recording(self, output_path: Path, format: str = "mp4", 
...
class VideoRecorder:
    ...
    def __init__(self, output_path: Path, format: str, resolution: str, 
                 fps: int, quality: str):
...
def validate_stream_config(config: StreamConfig) -> List[str]:
    ...
    if platform_config:
        ...
        if config.bitrate > max_bitrate:
            errors.append(f"Bitrate too high for {platform_config['name']} (maximum {max_bitrate} kbps)")
