...
class FaceAlignerTrainerApp:
    ...
    def get_random_warp(self) -> bool:
        return self._random_warp
    ...
    def get_autosave_period(self):
        return self._autosave_period
    ...
    def get_is_training(self) -> bool:
        return (
            self._req_is_training
            if self._req_is_training is not None
            else self._is_training
        )
    ...
    def set_loss_history(self, lh):
        self._loss_history = lh

    def load(self):
        self._model_data = model_data = (
            torch.load(self._model_data_path, map_location="cpu")
            if self._model_data_path.exists()
            else {}
        )

        self.set_device_info(
            lib_torch.get_device_info_by_index(model_data.get("device_index", -1))
        )
            ...
            if load:
                ...
                if model_state_dict is not None:
                    try:
                        ...
                        model_optimizer_state_dict = self._model_data.get(
    ...
    def save(self):
        if self._model_data is not None:
            d = {
                "device_index": self._device_info.get_index(),
    ...
    def export(self):
        ...
        torch.onnx.export(
            ...
            (
                torch.from_numpy(
                    np.zeros(
                        (1, 3, self._resolution, self._resolution), dtype=np.float32
                    )
                ),
            ),
    ...
    def preview_thread_proc(self):
        while not self._is_quit:
            ...
            if preview_data is not None:
                ...
                shift_mat = (
                    lib_math.Affine2DUniMat(data.shift_uni_mats[n])
                    .invert()
                    .to_exact_mat(W, H, W, H)
                )
                shift_mat_pred = (
                    lib_math.Affine2DUniMat(preview_data.shift_uni_mats_pred[n])
                    .invert()
                    .to_exact_mat(W, H, W, H)
                )

                img_aligned_unshifted = cv2.warpAffine(
                    img_aligned_shifted, shift_mat, (W, H)
                )
                img_aligned_unshifted_pred = cv2.warpAffine(
                    img_aligned_shifted, shift_mat_pred, (W, H)
                )
                ...
                screen = np.concatenate(
                    [
                        img_aligned,
                        img_aligned_shifted,
                        img_aligned_unshifted,
                        img_aligned_unshifted_pred,
                    ],
                    1,
                )
            ...
            if viewing_data is not None:
                ...
                img_aligned_shifted = viewing_data.img_aligned_shifted[n].transpose(
                    (1, 2, 0)
                )
    ...
    def main_loop(self):
        while not self._is_quit:
            ...
            if self._is_device_changed:
                self._is_device_changed = False
                self._model.to(self._device)
                self._model_optimizer.load_state_dict(
                    self._model_optimizer.state_dict()
                )
    ...
    def main_loop_training(self):
        ...
        if (
            self._is_training
            or self._is_previewing_samples
            or self._ev_request_preview.is_set()
        ):
            training_data = self._training_generator.get_next_data(wait=False)
            if (
                training_data is not None
                and training_data.resolution == self.get_resolution()
            ):  # Skip if resolution is different, due to delay
                if self._is_training:
                    self._model_optimizer.zero_grad()

                if self._ev_request_preview.is_set() or self._is_training:
                    # Inference for both preview and training
                    img_aligned_shifted_t = torch.tensor(
                        training_data.img_aligned_shifted
                    ).to(self._device)
                    shift_uni_mats_pred_t = self._model(
                        img_aligned_shifted_t
                    )  # .view( (-1,2,3) )

                if self._is_training:
                    # Training optimization step
                    shift_uni_mats_t = torch.tensor(training_data.shift_uni_mats).to(
                        self._device
                    )
                    loss_t = (
                        shift_uni_mats_pred_t - shift_uni_mats_t
                    ).square().mean() * 10.0
                    loss_t.backward()
                ...
                if self._ev_request_preview.is_set():
                    ...
                    pd.shift_uni_mats_pred = (
                        shift_uni_mats_pred_t.detach().cpu().numpy()
                    )
    ...
    def get_main_dlg(self):
        ...
        return dc.DlgChoices(
            [
                dc.DlgChoice(
                    short_name="sgm",
                    row_def="| Sample generator menu.",
                    on_choose=lambda dlg: self.get_sample_generator_dlg(
                        dlg
                    ).set_current(),
                ),
                dc.DlgChoice(
                    short_name="d",
                    row_def=f"| Device | {self._device_info}",
                    on_choose=lambda dlg: self.get_training_device_dlg(
                        dlg
                    ).set_current(),
                ),
                dc.DlgChoice(
                    short_name="lr",
                    row_def=f"| Learning rate | {self.get_learning_rate()}",
                    on_choose=lambda dlg: self.get_learning_rate_dlg(
                        parent_dlg=dlg
                    ).set_current(),
                ),
                dc.DlgChoice(
                    short_name="i",
                    row_def=f"| Iteration | {self.get_iteration()}",
                    on_choose=lambda dlg: self.get_iteration_dlg(
                        parent_dlg=dlg
                    ).set_current(),
                ),
                dc.DlgChoice(
                    short_name="l",
                    row_def=f"| Print loss history | Last loss = {last_loss:.5f} ",
                    on_choose=self.on_main_dlg_print_loss_history,
                ),
                dc.DlgChoice(
                    short_name="p",
                    row_def="| Show current preview.",
                    on_choose=lambda dlg: (
                        self._ev_request_preview.set(),
                        dlg.recreate().set_current(),
                    ),
                ),
                dc.DlgChoice(
                    short_name="t",
                    row_def=f"| Training | {self.get_is_training()}",
                    on_choose=lambda dlg: (
                        self.set_is_training(not self.get_is_training()),
                        dlg.recreate().set_current(),
                    ),
                ),
                dc.DlgChoice(
                    ...
                    row_def="| Reset model.",
                    on_choose=lambda dlg: (
                        self._ev_request_reset_model.set(),
                        dlg.recreate().set_current(),
                    ),
                ),
                dc.DlgChoice(
                    short_name="export",
                    row_def="| Export model.",
                    on_choose=lambda dlg: self._ev_request_export_model.set(),
                ),
                dc.DlgChoice(
                    short_name="se",
                    row_def=f"| Autosave period | {self.get_autosave_period()} minutes",
                    on_choose=lambda dlg: self.get_autosave_period_dlg(
                        dlg
                    ).set_current(),
                ),
                dc.DlgChoice(
    ...
    def on_main_dlg_print_loss_history(self, dlg):
        ...
        for key in self._loss_history.keys():
            ...
            if lh_len >= max_lines:
                ...
                lh_ar_max, lh_ar_min, lh_ar_mean, lh_ar_median = (
                    lh_ar.max(-1),
                    lh_ar.min(-1),
                    lh_ar.mean(-1),
                    np.median(lh_ar, -1),
                ...
                print(
                    "\n".join(
                        f"max:[{max_value:.5f}] min:[{min_value:.5f}] mean:[{mean_value:.5f}] median:[{median_value:.5f}]"
                        for max_value, min_value, mean_value, median_value in zip(
                            lh_ar_max, lh_ar_min, lh_ar_mean, lh_ar_median
                        )
                    )
                )
    ...
    def get_sample_generator_dlg(self, parent_dlg):
        return dc.DlgChoices(
            [
                dc.DlgChoice(
                    short_name="v",
                    row_def=f"| Previewing samples | {self._is_previewing_samples}",
                    on_choose=self.on_sample_generator_dlg_previewing_last_samples,
                ),
                dc.DlgChoice(
                    short_name="rw",
                    row_def=f"| Random warp | {self.get_random_warp()}",
                    on_choose=lambda dlg: (
                        self.set_random_warp(not self.get_random_warp()),
                        dlg.recreate().set_current(),
                    ),
                ),
                dc.DlgChoice(
                    short_name="r",
                    row_def=f"| Running | {self._training_generator.is_running()}",
                    on_choose=lambda dlg: (
                        self._training_generator.set_running(
                            not self._training_generator.is_running()
                        ),
                        dlg.recreate().set_current(),
                    ),
                ),
    ...
    def get_autosave_period_dlg(self, parent_dlg):
        return dc.DlgNumber(
            is_float=False,
            min_value=1,
            on_value=lambda dlg, value: (
                self.set_autosave_period(value),
                parent_dlg.recreate().set_current(),
            ),
    ...
    def get_training_device_dlg(self, parent_dlg):
        return DlgTorchDevicesInfo(
            on_device_choice=lambda dlg, device_info: (
...
class DlgTorchDevicesInfo(dc.DlgChoices):
    def __init__(
        self,
        on_device_choice: Callable = None,
        ...
        super().__init__(
            choices=[
                dc.DlgChoice(
                    short_name=f"{device.get_index()}" if not device.is_cpu() else "c",
                    row_def=f"| {str(device.get_name())} "
                    + (
                        f"| {(device.get_total_memory() / 1024**3) :.3}Gb"
                        if not device.is_cpu()
                        else ""
                    ),
                    on_choose=(
                        (lambda dlg, i=i: on_device_choice(dlg, devices[i]))
                        if on_device_choice is not None
                        else None
                    ),
                )
            ...
            on_multi_choice=(
                (lambda idxs: on_device_multi_choice([devices[idx] for idx in idxs]))
                if on_device_multi_choice is not None
                else None
            ),
            on_recreate=on_recreate,
            on_back=on_back,
            top_rows_def=top_rows_def,
            bottom_rows_def=bottom_rows_def,
        )
