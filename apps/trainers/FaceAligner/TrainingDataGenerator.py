    ...
    def get_next_data(self, wait: bool) -> Union[Data, None]:
        ...
        while True:
            for _ in range(self.get_process_count()):
                process_id, self._datas_counter = (
                    self._datas_counter % len(self._datas),
                    self._datas_counter + 1,
                )
            ...
            if self._sent_buffers_count < 2:
                ...
                if self._n_batch < batch_size:
                    ...
                    while True:
                        ...
                        if flmrks1 is None:
                            print(
                                f"Corrupted faceset, no FLandmarks2D for UFaceMark {ufm1.get_uuid()}"
                            )
                            continue
                        ...
                        if uimg1 is None:
                            print(
                                f"Corrupted faceset, no UImage for UFaceMark {ufm1.get_uuid()}"
                            )
                            continue
                        ...
                        if img1 is None:
                            print(
                                f"Corrupted faceset, no image in UImage {uimg1.get_uuid()}"
                            )
                            continue
                        ...
                        _, img_to_face_uni_mat1 = flmrks1.calc_cut(
                            img1.shape[0:2], face_coverage, resolution
                        )

                        fw1 = lib_face.FaceWarper(
                            img_to_face_uni_mat1,
                        ...
                        img_aligned_shifted = (
                            fw1.transform(
                                img1, resolution, random_warp=self._random_warp
                            ).astype(np.float32)
                            / 255.0
                        )
                        ...
                        if rnd.randint(2) == 0:
                            ip.hsv(
                                ...
                                rnd.uniform(-0.5, 0.5),
                                rnd.uniform(-0.5, 0.5),
                                mask=lib_sd.random_circle_faded_multi(
                                    (resolution, resolution),
                                    complexity=random_mask_complexity,
                                ),
                            )
                        else:
                            ip.levels(
                        ...
                        if rnd.randint(2) == 0:
                            if rnd.randint(100) < sharpen_chance:
                                if rnd.randint(2) == 0:
                                    ip.box_sharpen(
                                        size=rnd.randint(1, 11),
                                        power=rnd.uniform(0.5, 5.0),
                                        mask=lib_sd.random_circle_faded_multi(
                                            (resolution, resolution),
                                            complexity=random_mask_complexity,
                                        ),
                        ...
                        if np.random.randint(2) == 0:
                            if rnd.randint(100) < reresize_chance:
                                ip.reresize(
                                    rnd.uniform(0.0, 0.75),
                                    interpolation=ip.Interpolation.LINEAR,
                                    mask=lib_sd.random_circle_faded_multi(
                                        (resolution, resolution),
                                        complexity=random_mask_complexity,
                                    ),
                        ...
                        self._shift_mat_list.append(
                            fw1.get_aligned_random_transform_mat()
                        )
                        self._n_batch += 1
                        break
                ...
                if self._n_batch == batch_size:
                    ...
                    data.img_aligned = np.array(self._img_aligned_list).transpose(
                        (0, 3, 1, 2)
                    )
                    data.img_aligned_shifted = np.array(
                        self._img_aligned_shifted_list
                    ).transpose((0, 3, 1, 2))
